...
public class Aiecommitmentintfr extends javax.swing.JInternalFrame {
    ...
    public Aiecommitmentintfr(java.sql.Connection connDb, org.netbeans.lib.sql.pool.PooledConnectionSource pconnDB) {
...
//        getListThread = new java.lang.Thread(this, "Update_petty_cash");
    ...
    private void initComponents() {
        ...
        */
        ...
        dealerCmbx = new javax.swing.JComboBox();
        ...
        */
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Order/Imprest  Votebook Commitment");
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Supplier Code", "Supplier", "Document Type", "Commitment Type", "LPO/LSO/IMPREST. No", "Amount", "Remove"
            }
        ) {
        ...
        javax.swing.table.TableColumn teditor = this.jTable1.getColumn("Document Type");
    ...
    */
    ...
    searchButton.setPreferredSize(new java.awt.Dimension(19, 19));
    ...
    orderPaymentsChkbx.setText("LPO/LSO Commitments");
    orderPaymentsChkbx.addItemListener(new java.awt.event.ItemListener() {
        public void itemStateChanged(java.awt.event.ItemEvent evt) {
            orderPaymentsChkbxItemStateChanged(evt);
        }
    });
    ...
    dealerCmbx.setModel(com.afrisoftech.lib.ComboBoxModel.ComboBoxModel(connectDB, "SELECT distinct dealer FROM ac_bills WHERE final_voucher_no = ''  AND commited = false ORDER BY 1"));
    dealerCmbx.setBorder(javax.swing.BorderFactory.createTitledBorder("Select Imprest Holder"));
    dealerCmbx.setMinimumSize(new java.awt.Dimension(135, 20));
    dealerCmbx.addActionListener(new java.awt.event.ActionListener() {
        public void actionPerformed(java.awt.event.ActionEvent evt) {
            dealerCmbxActionPerformed(evt);
        }
    });
    gridBagConstraints = new java.awt.GridBagConstraints();
    gridBagConstraints.gridx = 5;
    ...
    getContentPane().add(dealerCmbx, gridBagConstraints);

    dealerCodeTxt.setBorder(javax.swing.BorderFactory.createTitledBorder("Dealer Code"));
    gridBagConstraints = new java.awt.GridBagConstraints();
    gridBagConstraints.gridx = 3;
    gridBagConstraints.gridy = 8;
    gridBagConstraints.fill = java.awt.GridBagConstraints.BOTH;
    gridBagConstraints.weightx = 1.0;
    gridBagConstraints.weighty = 1.0;
    getContentPane().add(dealerCodeTxt, gridBagConstraints);
        ...
        try {
            ...
            for (int i = 0; i < jTable1.getRowCount(); i++) {
                if ((jTable1.getModel().getValueAt(i, 2) != null)) {
                    ...
                    java.sql.PreparedStatement pstmt = connectDB.prepareStatement("UPDATE ac_bills set commited = true WHERE voucher_no ilike '"+jTable1.getValueAt(i, 4).toString()+"'");
                
                    pstmt.executeUpdate();
                    
//                    com.afrisoftech.lib.DocumentTracer.setDocumentLocation(connectDB, jTable1.getValueAt(i, 4).toString(), commentsTxt.getText(), "EXPENDITURE_VOTEBOOK", documentTargetCmbx.getSelectedItem().toString());
                }
                ...
                if (jTable1.getModel().isCellEditable(jTable1.getSelectedRow(), 0)) {
                    if (jTable1.getModel().isCellEditable(jTable1.getSelectedRow(), 0) && !(jTable1.getModel().getValueAt(jTable1.getSelectedRow(), 0).toString().equalsIgnoreCase(questmark))) {
                        try {
                            java.sql.Statement pstmtx = connectDB.createStatement();
                            // java.sql.ResultSet rs1x = pstmtx.executeQuery("SELECT product_id,product, strength,round(selling_price, 0) as selling_price, units FROM stockprices WHERE product_id ILIKE '" + jTable13.getValueAt(jTable13.getSelectedRow(), 0) + "'");
                            java.sql.ResultSet rs1x = pstmtx.executeQuery("SELECT first_name||' '||middle_name||' '||last_name FROM master_file WHERE employee_no = '" + jTable1.getValueAt(jTable1.getSelectedRow(), 0) + "'");
...
//            rss.close();
    ...
    private void dealerCmbxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_dealerCmbxActionPerformed
        if (imprestChkbx.isSelected()) {
            dealerCodeTxt.setText(null);
            try {
                java.sql.PreparedStatement pstmt = connectDB.prepareStatement("SELECT DISTINCT dealer_code from ac_bills WHERE dealer = ?");
                pstmt.setString(1, dealerCmbx.getSelectedItem().toString());
                java.sql.ResultSet rset = pstmt.executeQuery();
                while (rset.next()) {
                    dealerCodeTxt.setText(rset.getString(1));
                }

                //   if (dealerCodeTxt.getText() != null) {
                jTable1.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "SELECT dealer_code, dealer, 'IMPREST', 'SINGLE', voucher_no, credit,false as Remove from ac_bills WHERE dealer_code = '"+dealerCodeTxt.getText()+"' AND final_voucher_no = '' ORDER by 2"));

            } catch (SQLException ex) {
                ex.printStackTrace();
                javax.swing.JOptionPane.showMessageDialog(this, ex.getMessage());
                Logger.getLogger(Aiecommitmentintfr.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }//GEN-LAST:event_dealerCmbxActionPerformed

    private void removebtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_removebtnActionPerformed

             for (int k = 0; k < jTable1.getRowCount(); k++) {
                 if(jTable1.getModel().getValueAt(k, 6).toString().trim().equalsIgnoreCase("true")){
                    // jTable1.removeRowSelectionInterval(k, 6);
                     
                    javax.swing.table.DefaultTableModel defTableModel = (javax.swing.table.DefaultTableModel)jTable1.getModel();
                     defTableModel.removeRow(k);
                 }
    ...
    private void imprestChkbxItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_imprestChkbxItemStateChanged
       if(imprestChkbx.isSelected()==true){
          removebtn.setEnabled(true);
       }else if(imprestChkbx.isSelected()==false){
          removebtn.setEnabled(false);
       }
        
    }//GEN-LAST:event_imprestChkbxItemStateChanged

    private void orderPaymentsChkbxItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_orderPaymentsChkbxItemStateChanged
        // TODO add your handling code here:
          if(imprestChkbx.isSelected()==true){
          removebtn.setEnabled(false);
       }else if(imprestChkbx.isSelected()==false){
    ...
    }//GEN-LAST:event_orderPaymentsChkbxItemStateChanged
    ...
    private void returnDocumentBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_returnDocumentBtnActionPerformed
        ...
        com.afrisoftech.lib.DocumentTracer.setDocumentLocation(connectDB, jTable1.getValueAt(jTable1.getSelectedRow(), 4).toString(), commentsTxt.getText(), "EXPENDITURE_VOTEBOOK", documentTargetCmbx.getSelectedItem().toString());
        // TODO add your handling code here:
    ...
    private javax.swing.JComboBox dealerCmbx;
