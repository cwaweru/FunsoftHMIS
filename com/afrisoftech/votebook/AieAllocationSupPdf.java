...
public class AieAllocationSupPdf implements java.lang.Runnable {
    ...
    public void AieAllocationSupPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate) {
        connectDB = connDb;
        beginDate = begindate;
        endDate = endate;
                ...
                try {
                    ...
                    try {
                        ...
                        for (int i = 0; i < listofAct2.length; i++) {
                            ...
                            int headerwidths[] = {10, 30, 50, 30, 15,};
                            ...
                            try {
                                ...
                                java.sql.ResultSet rset15 = st15.executeQuery("SELECT at.v_class||'-'||at.vote||'-'||at.subvote"
                                        + "||'-'||at.subhead||'-'||at.subitem||'-'||at.donorcode||'-'||at.donoritem,at.subitem,SUM(at.allocated_amount) AS amt,"
                                        + "INITCAP(sb.subitemname) as sub,at.voucher_no,INITCAP(at.supplier) FROM ac_aie_commitment at, subitems sb WHERE "
                                        + "at.v_class = '" + listofAct2[i].toString() + "' AND at.subitem = sb.subitem GROUP BY 1,2,4,5,6  HAVING SUM(at.allocated_amount) > 0  ORDER BY 1");
                                ...
                                while (rset11.next()) {
                                    ...
                                    phrase = new Phrase("PAYEE/VENDOR", pFontHeader);
                                    ...
                                    while (rset15.next()) {
                                        ...
                                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                        phrase = new Phrase(dbObject.getDBObject(rset15.getObject(5), "-").toUpperCase(), pFontHeader1);
                                        table.addCell(phrase);
                                        phrase = new Phrase(dbObject.getDBObject(rset15.getObject(1), "-").toUpperCase(), pFontHeader1);
                                        table.addCell(phrase);
                                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                        phrase = new Phrase(dbObject.getDBObject(rset15.getObject(4), "-").toUpperCase(), pFontHeader1);
                                        table.addCell(phrase);

                                        phrase = new Phrase(dbObject.getDBObject(rset15.getObject(6), "-").toUpperCase(), pFontHeader1);
                                        table.addCell(phrase);
