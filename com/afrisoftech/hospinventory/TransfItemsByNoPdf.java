                ...
                try {
                    ...
                    try {
                        ...
                        for (int j = 0; j < listofStaffNos.length; j++) {
                            ...
                            int headerwidths[] = {40,20,15,20,25};
                            ...
                            try {
                                ...
                                java.sql.ResultSet rset2 = st2.executeQuery("select distinct store_name,units,sub_store,issiued_to,user_name,trans_date::date from st_sub_stores WHERE transaction_no = '"+listofStaffNos[j]+"' and receiving > 0 GROUP BY store_name,units,sub_store,issiued_to,user_name,trans_date");
                                ...
                                table.getDefaultCell().setColspan(1);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Date : "+dbObject.getDBObject(rset2.getObject(6), "-"), pFontHeader);
                                table.addCell(phrase);
                                }
                            ...
                            try {
                                ...
                                java.sql.ResultSet rset = st.executeQuery("select initcap(ss.item),ss.units,round(sum(ss.issuing)),sum(buying_price)::numeric(10,2),round(sum(ss.total)) from st_sub_stores ss WHERE ss.transaction_no = '"+listofStaffNos[j]+"' AND ss.issuing > 0 group by ss.item,ss.units ORDER BY ss.item");
                                //  java.sql.ResultSet rsetTotals = st2.executeQuery("SELECT SUM(net_value) from orders WHERE received = false");// WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                                java.sql.Statement stt = connectDB.createStatement();
