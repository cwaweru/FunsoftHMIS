 ...
 */
public class ChiefProcurementOfficerApprovalsIntfr extends javax.swing.JInternalFrame {
    ...
    public ChiefProcurementOfficerApprovalsIntfr(java.sql.Connection connDb, org.netbeans.lib.sql.pool.PooledConnectionSource pconnDB) {

        connectDB = connDb;

        pConnDB = pconnDB;
        initComponents();
        setFixedColumns.DisallowReordering(chief_tbl);
        ...
        try {
            ...
            for (int k = 0; k < chief_tbl.getRowCount(); k++) {
                if (chief_tbl.getValueAt(k, 0) != null) {
                    if (Boolean.valueOf(chief_tbl.getModel().getValueAt(k, 9).toString()) == java.lang.Boolean.TRUE) {

                        String comment;
                        if (chief_tbl.getValueAt(k, 8) == null) {
                            comment = "";
                        } else {
                            comment = chief_tbl.getValueAt(k, 8).toString();
                        }

                        java.sql.PreparedStatement pstmt81212 = connectDB.prepareStatement(""
                                + "update st_orders set chief_approval=true,chief='" + correct_name + "',chief_date=now(),chief_remark='" + comment + "' where dispatch=false and ordered=false and chief_approval=false and aie_holder_approval=false and sad_approval=false  and miu_approval=false  and approval_rejected is null and quotation_no='" + TenderTxt.getText() + "' and order_no='" + lpo + "' and item ='" + chief_tbl.getModel().getValueAt(k, 2).toString() + "' and code='" + chief_tbl.getModel().getValueAt(k, 1).toString() + "'");
   ...
   private Double getLimitSAD(){
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try {
            ...
            for (int k = 0; k < chief_tbl.getRowCount(); k++) {
                if (chief_tbl.getValueAt(k, 0) != null) {
                    if (Boolean.valueOf(chief_tbl.getValueAt(k, 9).toString()) == java.lang.Boolean.TRUE) {
...
//        }        // TODO add your handling code here:
    ...
    private void ApprovalTblMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ApprovalTblMouseClicked
        try {
            ...
            java.sql.ResultSet rsets = stmtss.executeQuery("SELECT distinct ordering_store,quotation_no,requisition_no FROM st_orders WHERE order_no ='" + lpo + "' and dispatch=false and ordered=false and chief_approval=false and aie_holder_approval=false and sad_approval=false  and miu_approval=false  and approval_rejected is null");
            ...
            chief_tbl.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "SELECT distinct o.date_due,o.code,o.item,o.item_strength,o.units, o.quantity, o.unit_price,o.net_value,null Remarks,false Approve  FROM st_orders o,st_item_to_quote q where     o.order_no='" + lpo + "' and  q.tender_no='" + TenderTxt.getText() + "' and q.tender_no=o.quotation_no and o.item=q.item_description  and  o.approval_rejected is null and o.chief_approval=false "));
            connectDB.commit();
            connectDB.setAutoCommit(true);
    ...
    private void jTextField1CaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_jTextField1CaretUpdate
        ...
        if (jTextField1.getCaretPosition() > 3) {
           ...
           // ApprovalTbl.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB,"SELECT distinct order_no as LPO_LSO_NO,supplier as supplier_name from st_orders st where dispatch=false and ordered=false and chief_approval=false and aie_holder_approval=false and sad_approval=false  and miu_approval=false  and approval_rejected is null and order_no is not null group by supplier,order_no  order by order_no"));
            ...
            ApprovalTbl.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "SELECT distinct order_no as LPO_LSO_NO,supplier as supplier_name from st_orders  where dispatch=false and ordered=false and chief_approval=false and aie_holder_approval=false and sad_approval=false  and miu_approval=false and order_no is not null  group by supplier,order_no  order by supplier"));
        }
