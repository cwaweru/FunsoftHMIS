            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {10,30,12,10,10,8,8,14};
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = st.executeQuery("select ss.trans_date::date,ss.item,ROUND(sum(ss.issuing)),initcap(ss.user_name),ss.store_name,ss.sub_store,sum(ss.total/issuing),sum(ss.total) from st_sub_stores ss WHERE ss.transaction_no ILIKE 'T%' and ss.trans_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ss.issuing > 0 GROUP BY ss.trans_date,ss.item,initcap(ss.user_name),ss.sub_store,ss.store_name HAVING sum(ss.issuing) > 0 ORDER BY ss.trans_date");
