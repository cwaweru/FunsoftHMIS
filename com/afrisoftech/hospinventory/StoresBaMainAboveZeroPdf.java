...
public class StoresBaMainAboveZeroPdf implements java.lang.Runnable {
   ...
   // public void StoresBalPdf_1(java.sql.Connection connDb,java.lang.String begindate,java.lang.String endate,java.lang.String cbox) {
      public void StoresBaMainlAboveZeroPdf(java.sql.Connection connDb, java.util.Date endate, java.util.Date begindate,java.lang.String cbox) {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                // java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(receiving-issuing) from stock_movement_summary WHERE item = ? AND store_name ilike  '"+CBox+"' AND trans_date <= '"+endDate+"'");
                                  ...
                                  java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(qty)::NUMERIC(10,0) AS BALANCE from stock_balance_qty where department ilike '"+CBox+"' and description = ? AND dates <= '"+endDate+"'  GROUP BY description having sum(qty) >0");
                                   java.sql.PreparedStatement pset = connectDB.prepareStatement("select distinct buying_price from st_stock_item WHERE description ilike ? and department ilike  '"+CBox+"'");
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
          ...
          //  java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT item FROM stock_movement_summary  WHERE store_name ilike '"+CBox+"' AND trans_date <= '"+endDate+"' order by item");
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT initcap(item||' '||strength) as item FROM st_substore_item WHERE sub_store ilike '"+CBox+"' AND trans_date <= '"+endDate+"' order by item");
