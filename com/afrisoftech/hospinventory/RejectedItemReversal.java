 ...
 * @author root
 */
public class RejectedItemReversal extends javax.swing.JInternalFrame {
    ...
    public RejectedItemReversal(java.sql.Connection connDb, org.netbeans.lib.sql.pool.PooledConnectionSource pconnDB) {
    ...
    private void initComponents() {
        ...
        */
        ...
        gridBagConstraints.gridx = 4;
        gridBagConstraints.gridy = 4;
        getContentPane().add(jTextField61, gridBagConstraints);
        ...
        jTable2.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "select invoice_no as dNote_No,supplier from st_stock_cardex where grn_no='rejected' and approved=false"));
...
// TODO add your handling code here:
    ...
    private void jSearchTable21MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jSearchTable21MouseClicked
        ...
        /*
         * try {
         * datePicker1.setDate(sdf.parse(jSearchTable21.getValueAt(jSearchTable21.getSelectedRow(),
         * 2).toString())); } catch (java.text.ParseException pe) {
     ...
     /*
         * jTable1.setValueAt(jSearchTable21.getValueAt(jSearchTable.getSelectedRow(),0),jTable1.getSelectedRow(),
         * 4);
         * jTable1.setValueAt(jSearchTable21.getValueAt(jSearchTable.getSelectedRow(),1),jTable1.getSelectedRow(),
         * 5);
         * jTable1.setValueAt(jSearchTable21.getValueAt(jSearchTable.getSelectedRow(),2),jTable1.getSelectedRow(),
         * 6);
         * jTable1.setValueAt(jSearchTable21.getValueAt(jSearchTable.getSelectedRow(),3),jTable1.getSelectedRow(),
         * 7);
         * jTable1.setValueAt(jSearchTable21.getValueAt(jSearchTable.getSelectedRow(),4),jTable1.getSelectedRow(),
         * 8);
         */
         ...
         * '"+jSearchTable.getValueAt(jSearchTable.getSelectedRow(),0)+"'");
         * while (rs1.next()) j = rs1.getInt(1);
         *
         * } catch(java.sql.SQLException sqlex){
         * System.out.println(sqlex.getMessage()); }
         *
         *
         * jSearchDialog.dispose(); jTextField111.setText("");
         *
         */
    ...
    private void jTextField1131CaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_jTextField1131CaretUpdate
        ...
        if (this.jTextField1131.getCaretPosition() < 3) {
            ...
            jSearchTable21.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "select distinct invoice_no,supplier,date,sum(debit-quantity_ordered) as amt from st_stock_cardex where invoice_no ILIKE '" + jTextField1131.getText() + "%' and transaction_type ILIKE 'Receiving%' GROUP BY invoice_no,supplier,date ORDER BY invoice_no"));
    ...
    private void jCheckBox2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCheckBox2ActionPerformed
        ...
        //  jComboBox1.setM
        jComboBox2.setModel(com.afrisoftech.lib.ComboBoxModel.ComboBoxModel(connectDB, "select '-' union all select store_name from st_stores"));
    ...
    private void jCheckBox1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCheckBox1ActionPerformed
        ...
        //  jComboBox1.setM
        //   jComboBox2.setModel(new org.netbeans.lib.sql.models.ComboBoxModel(crset3, "store_name", null, null, null));
        jComboBox2.setModel(com.afrisoftech.lib.ComboBoxModel.ComboBoxModel(connectDB, "select '-' as store_name union all select store_name from st_main_stores"));
        jComboBox2.addActionListener(new java.awt.event.ActionListener() {
...
//            jSearchTable.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, " select item,quantity_received as qty,PRICE_PER_ITEM AS PRICE,debit as total,item_code,order_no FROM st_stock_cardex WHERE approved=false and  item ILIKE '%" + jTextField111.getText() + "%' AND store ilike '%" + jComboBox2.getSelectedItem().toString() + "%' and invoice_no = '" + jTextField91.getText() + "'  "
//                    + " group by item,quantity_received,PRICE_PER_ITEM ,debit ,item_code,order_no having sum(debit-quantity_ordered)>0 order by ITEM"));
jSearchTable.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "select item,PRICE_PER_ITEM AS PRICE,item_code,order_no FROM st_stock_cardex WHERE approved=false and grn_no='rejected' AND store ilike '%" + jTextField111.getText() + "%' AND store ilike '%" + jComboBox2.getSelectedItem().toString() + "%'  and invoice_no =  '" + jTextField91.getText() + "' group by item,PRICE_PER_ITEM ,item_code,order_no having sum(debit-quantity_ordered)>0 order by ITEM"));
            /*
...
//                    + " group by item,quantity_received,PRICE_PER_ITEM ,debit ,item_code,order_no having sum(debit-quantity_ordered)>0 order by ITEM"));
        ...
        try { 
         ...
         java.sql.ResultSet rs12 = pstmt1.executeQuery("select item,quantity_received,PRICE_PER_ITEM ,debit ,item_code,order_no "
                 + "                                     FROM st_stock_cardex WHERE"
                 + " approved=false and grn_no='rejected' and item_code='"+jSearchTable.getValueAt(jSearchTable.getSelectedRow(), 2)+"' and  item='"+jSearchTable.getValueAt(jSearchTable.getSelectedRow(), 0)+"' AND store ilike '%" + jComboBox2.getSelectedItem().toString() + "%' and invoice_no = '" + jTextField91.getText() + "'");
          rs12.next();
...
//        jTable1.setValueAt("1", jTable1.getSelectedRow(), 2);
         ...
         *
         * java.sql.ResultSet rs1 = pstmt1.executeQuery("select count(product)
         * from st_stock_cardex where item =
         * '"+jSearchTable.getValueAt(jSearchTable.getSelectedRow(),0)+"'");
         * while (rs1.next()) j = rs1.getInt(1); if (j > 0){
         *
         * java.sql.ResultSet rs = pstmt.executeQuery("select
         * item,units,'1',quantity_received,price_per_item,'0.00',credit,'0.00','0.00',debit
         * from st_stock_cardex where item =
         * '"+jSearchTable.getValueAt(jSearchTable.getSelectedRow(),0)+"' and
         * invoice_no = '"+jTextField91.getText()+"' and date =
                    ...
                    try {
                        ...
                        for (int i = 0; i < jTable1.getRowCount(); i++) {
                            if (jTable1.getModel().getValueAt(i, 9) != null) {
                                ...
                                pstmt.executeUpdate();
                                String ter="rejectedNReversed";
  java.sql.PreparedStatement pstmt81212 = connectDB.prepareStatement("update st_stock_cardex set approved=true,grn_no='"+ter+"' where"
                                 + " delivery_note_no='" + jTextField91.getText()+ "' "
                                 + " and invoice_no='" +jTextField91.getText()+ "' "
                                 + " and order_no='" +LPONO.getText()+ "' "
                                 + " and supplier='" + jTextField7.getText()+ "' "
                          ...
                          pstmt81212.executeUpdate();
                                ...
                                pstmt11.setObject(8, transNo + "- Reversal of GRN No. " + jTextField91.getText());
                                ...
                                pstmt11.setObject(13, jComboBox2.getSelectedItem().toString());
                        ...
                        java.sql.ResultSet rs1d1 = pst1d1.executeQuery("select cost_of_sale from pb_departments where department_name ILIKE '" + jComboBox2.getSelectedItem() + "%'");
                        while (rs1d1.next()) {
                        ...
                        pstmt1q1.setString(6, jTextField91.getText());
    ...
    connectDB.setAutoCommit(false);
        java.sql.Statement stmtss=connectDB.createStatement();
                java.sql.ResultSet rsets = stmtss.executeQuery("select distinct invoice_no,supplier,date,sum(debit-quantity_ordered) as amt,order_no from st_stock_cardex where invoice_no ILIKE '" + jTable2.getModel().getValueAt(jTable2.getSelectedRow(), 0) + "%' and transaction_type ILIKE 'Receiving%' GROUP BY invoice_no,supplier,date,order_no ORDER BY invoice_no");
           
            while (rsets.next()) {
                 jTextField91.setText(rsets.getString(1));
                jTextField7.setText(rsets.getString(2));
                jTextField9.setText(rsets.getString(3));
                jTextField3.setText(rsets.getString(4));
                LPONO.setText(rsets.getString(5));
            }
