...
public class TransfersPerStoreGroupedPrevPdf implements java.lang.Runnable {
    ...
    com.lowagie.text.Font pFontHeader1 = FontFactory.getFont(FontFactory.HELVETICA, 9, Font.NORMAL);
     com.lowagie.text.Font pFontHeader4 = FontFactory.getFont(FontFactory.HELVETICA, 8, Font.ITALIC);
    ...
    public void TransfersPerStoreGroupedPrevPdf(java.sql.Connection connDb,java.util.Date begindate,java.util.Date endate,java.lang.String combox) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("Transfered Items list sorted by quantity descending - Page: ",pFontHeader4), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
                    ...
                    try {
                        ...
                        int headerwidths[] = {10,29,16,30,15};
                        ...
                        Phrase  phrase = new Phrase("TRANSFERED ITEMS SORTED BY QUANTITY", pFontHeader1);
                        table.addCell(phrase);
                        table.getDefaultCell().setColspan(5);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                        phrase = new Phrase("TRANSFERED FROM : " +bank.toUpperCase(), pFontHeader);
                        table.addCell(phrase);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = st.executeQuery("select ss.item,sum(ss.issuing),sum(ss.buying_price),round(sum(ss.buying_price*ss.issuing),2) as total from st_sub_stores_280808 ss WHERE transaction_no ILIKE 'T%' and ss.trans_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and ss.store_name ilike '"+bank+"' group by ss.item,ss.ISSUING having sum(ss.issuing)>0 ORDER BY 2 DESC");
                           //java.sql.ResultSet rset = st.executeQuery("select ss.trans_date::date,ss.item,sum(ss.quantity*-1),ss.user_name,upper(ss.store_name),ss.store_name,sum(bprice),round(sum(total),2) from transfers ss WHERE ss.trans_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and ss.store_name ilike '"+bank+"'group by ss.trans_date,ss.item,ss.user_name,ss.store_name,ss.issiued_to having sum(ss.quantity)<0 ORDER BY ss.item");
