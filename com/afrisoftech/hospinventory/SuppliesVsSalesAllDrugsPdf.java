...
public class SuppliesVsSalesAllDrugsPdf implements java.lang.Runnable {
    ...
    public void SuppliesVsSalesAllDrugsPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox) {
                  ...
                  */
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select '-' as sno, item,sum(qty)  ,sum(purchases),sum(sales),case when sum(sales) > 0 then sum(sales)*-1 else sum(purchases) end as balance  from drug_movt where date BETWEEN '"+beginDate+"' AND '"+endDate+"' and qty > 0 group by item order by item");// UNION ALL SELECT trans_date,store_name,user_name,issuing,total from st_sub_stores where (sub_store = 'Pharmacy' OR sub_store = 'IP Pharmacy') and item ilike '"+memNo+"' AND trans_date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                            java.sql.ResultSet rset11 = st11.executeQuery("select sum(purchases),sum(sales) from drug_movt");// UNION ALL SELECT trans_date,store_name,user_name,issuing,total from st_sub_stores where (sub_store = 'Pharmacy' OR sub_store = 'IP Pharmacy') and item ilike '"+memNo+"' AND trans_date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                            ...
                            table.getDefaultCell().setColspan(5);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                            phrase = new Phrase("Drug Movement(Supplies Vs Sales) for all stockables items", pFontHeader11);
                            table.addCell(phrase);
