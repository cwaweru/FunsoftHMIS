...
public class SubStoresBaMainlPdf implements java.lang.Runnable {
   ...
   // public void StoresBalPdf_1(java.sql.Connection connDb,java.lang.String begindate,java.lang.String endate,java.lang.String cbox) {
      public void SubStoresBaMainlPdf(java.sql.Connection connDb, java.util.Date endate, java.util.Date begindate,java.lang.String cbox) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                // java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(receiving-issuing) from stock_movement_summary WHERE item = ? AND store_name ilike  '"+CBox+"' AND trans_date <= '"+endDate+"'");
                                  ...
                                  java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(qty)::NUMERIC(10,0) AS BALANCE from stock_balance_qty where department ilike '"+CBox+"' and description = ? AND dates <= '"+endDate+"' GROUP BY description");
                                   java.sql.PreparedStatement pset = connectDB.prepareStatement("select distinct transfer_price from st_stock_prices WHERE product = ? and department ilike  '"+CBox+"'");
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
            ...
            java.sql.Statement stmt1 = connectDB.createStatement();
              java.sql.ResultSet rSet1 = stmt1.executeQuery("select initcap(description) as description from stock_balance_qty where department ilike '"+CBox+"' and dates <= '"+endDate+"' order by description");
         
             // java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT initcap(item) as item FROM st_substore_item WHERE sub_store ilike '"+CBox+"' AND trans_date <= '"+endDate+"' order by item");
