...
public class ItemsAvgConsPdf implements java.lang.Runnable {
    ...
    public void ItemsAvgConsPdf(java.sql.Connection connDb,java.lang.String cons,java.lang.String markup, java.lang.String dept) {
                    ...
                    try {
                        ...
                        */
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset11 = st.executeQuery("select round(sum(sc.issuing/(st.packaging*so.average_day)*st.buying_price)*'"+Consp+"'*'"+MarkUp+"',2) from st_sub_stores sc, st_stock_item st, st_ordering_constants so where sc.item = st.description and sc.issuing >0 and sc.trans_date > current_date - (so.average_day)::integer and st.department ilike '"+Department+"'");
                            while(rset11.next()){
                                Total = rset11.getDouble(1);
                            }
                            
                            java.sql.ResultSet rset = st.executeQuery("select distinct(st.description),st.buying_price,ROUND(sum(sc.issuing/st.packaging)),round(sum(sc.issuing/(st.packaging*so.average_day)),2) ,round(sum(sc.issuing/(st.packaging*so.average_day))*'"+Consp+"'),round(sum(sc.issuing/(st.packaging*so.average_day))*st.buying_price*'"+Consp+"',2) as sales from st_sub_stores sc, st_stock_item st, st_ordering_constants so WHERE sc.item = st.description and sc.issuing >0 and sc.trans_date > current_date - (so.average_day)::integer and st.department ilike '"+Department+"' group by st.description,st.buying_price order by sales desc");
