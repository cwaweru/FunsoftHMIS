...
public class TendersReceivedMtrhPdf implements java.lang.Runnable {
    ...
    public void TendersReceivedMtrhPdf(java.sql.Connection connDb, java.lang.String order, java.lang.String vendor) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            while (rset3.next()) {
                                ...
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                                phrase = new Phrase("TENDER DOCUMENT PRODUCT SPECIFICATION (Returned Copy)", pFontHeader1);
                                table1.addCell(phrase);
                                ...
                                table1.getDefaultCell().setColspan(10);
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("You are invited to submit Tender on materials listed below", pFontHeader5);
                                table1.addCell(phrase);
                                ...
                                table1.addCell(phrase);
                                phrase = new Phrase("(b) This tender should be submitted marked " + "Tender No. " + quoteNo + " for " + com.afrisoftech.lib.GetItemInfo.getITenderDesc(quoteNo, connectDB), pFontHeader5);
                                table1.addCell(phrase);
                                // table1.getDefaultCell().setColspan(5);
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("(c) Your Tender should indicate final costs per unit which includes delivery costs and applicable Taxes/Levies And and be addressed / Emailed to reach the buyer or be placed in the Tender box on or before : " + openDate, pFontHeader5);
                                table1.addCell(phrase);
                            ...
                            java.sql.ResultSet rset4 = st4.executeQuery("SELECT distinct item_number, item_code, item_description,units,quantity,unit_price, "
                                    + " (quantity*unit_price) as total,days_to_deliver, country_origin, remarks,discount,vat,(quantity*unit_price)-((discount+vat)*0.01)*(quantity*unit_price) as netvalue FROM st_floated_quotations WHERE "
                                    + "quotation_no ILIKE '" + OrderNo + "' AND supplier ILIKE '" + selectSupp + "' order by 1");// where supplier_name = 'Uchumi'member_no = '"+memNo+"'  AND date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                            cnt1 = 0;
                            while (rset4.next()) {
                                cnt += 1;
                                table3.getDefaultCell().setColspan(1);
                                ...
                                table3.getDefaultCell().setColspan(2);
                                phrase = new Phrase(com.afrisoftech.lib.CurrencyFormatter.getFormattedDouble(Double.parseDouble(rset4.getObject(7).toString())), pFontHeader5);
                                table3.addCell(phrase);
//                                
//                                ////start here 
