...
public class DetailedPrescPerPatDatePdf implements java.lang.Runnable {
    ...
    public void DetailedPrescPerPatDatePdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate, java.lang.String cashPoint, java.lang.String categ) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("PRESCRIPTIONS PER CATEGORY [ ORDERED BY DATE ]  - Period : "  +dateFormat.format(endDate11)+" - "+dateFormat.format(endDate1), pFontHeader);
                        ...
                        try {
                            ...
                            for (int k = 0; k < listofAct1.length; k++) {
                                ...
                                for (int i = 0; i < listofAct.length; i++) {
                                    ...
                                    java.sql.ResultSet rset2x = st52x.executeQuery("select distinct date_prescribed from hp_pharmacy where date_prescribed ilike '"+listofAct[i].toString()+"' and main_service = '"+CashPoint+"' group by date_prescribed");
                                    while (rset2x.next()) {
                                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    ...
                                    java.sql.ResultSet rset = st5.executeQuery("select initcap(ph.description),sum(ph.quantity)::numeric(4,0),sum(ph.amount),ph.patient_no from hp_pharmacy ph, st_stock_item st where ph.date_prescribed = '"+listofAct[i].toString()+"' AND ph.main_service ilike '"+CashPoint+"'  and ph.description ilike st.description and st.sub_cat_code ilike '"+Category+"%' GROUP BY ph.patient_no,ph.description ORDER BY ph.patient_no,ph.description");
                                    java.sql.Statement st52 = connectDB.createStatement();
                                    
                                    java.sql.ResultSet rset2 = st52.executeQuery("select count(description) from hp_pharmacy where date_prescribed = '"+listofAct[i].toString()+"' AND main_service ilike '"+CashPoint+"' group by description order by description");
                                    while (rset2.next()) {
