...
public class SendP9PerStaffPdf implements java.lang.Runnable {
                    ...
                    for (int j = 0; j < listofStaffNos.length; j++) {
                        try {
                            ...
                            try{
                                ...
                                java.sql.ResultSet rset = st.executeQuery("select distinct employee_no,upper(last_name),upper(first_name||' '||middle_name),pin_no,id_no from master_file where employee_no = '"+listofStaffNos[j]+"' order by employee_no");
                            ...
                            try {
                                ...
                                for (int l = 0; l < listofAct.length; l++) {
                                   ...
                                   // java.sql.ResultSet rset1 = st1.executeQuery("SELECT date,sum(amount) FROM posting WHERE staff_no = '"+listofStaffNos[j]+"' AND date = '"+listofAct[l]+"' AND allowance_deduction ILIKE 'Earning' GROUP BY date ORDER BY date ASC");
                                    ...
                                    System.err.println("SELECT date, sum(amount) FROM posting WHERE staff_no = '"+listofStaffNos[j]+"' AND date = '"+listofAct[l]+"' AND allowance_deduction ILIKE 'Non Cash%' AND description not ilike 'House value' GROUP BY date ORDER BY date ASC");
...
// }
            ...
            System.err.println("Send Pay9 via Email Now .............................................");

            biz.systempartners.claims.AdvancedSendFile1.SendFile(connectDB, filesArray, emailsArray, null, null, "P9 for : " + StaffNo + " " + com.afrisoftech.lib.StaffDetails.getStaffName(connectDB, StaffNo), "P9 for : " + StaffNo + " " + com.afrisoftech.lib.StaffDetails.getStaffName(connectDB, StaffNo));
