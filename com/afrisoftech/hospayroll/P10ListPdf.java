                    ...
                    try {
                        ...
                        try {
                            ...
                            for (int j = 0; j < listofStaffNos.length; j++) {
                                ...
                                //  java.sql.ResultSet rset = st.executeQuery("select mf.pin_no,first_name||' '||middle_name||' '|| last_name ,sum(tx.total_gross_pay),sum(tx.paye_tax) from tax_card  tx ,master_file mf WHERE mf.employee_no and tx.company_name ilike '"+bank+"' group by mf.pin_no,first_name||' '||middle_name||' '|| last_name ");
                                java.sql.ResultSet rset = st.executeQuery("select distinct pin_no,employee_no,upper(last_name||' '||first_name||' '||middle_name)from master_file where employee_no = '"+listofStaffNos[j]+"' order by employee_no");
                                ...
                                while (rset.next()) {
                                    while (rsetA.next()) {
                                        while (rsetB.next()) {
                                            ...
                                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                            phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rsetB.getString(1)),pFontHeader1);
                                            table.addCell(phrase);
                                            Net1 = Net1 + rsetB.getDouble(1);
                                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                            phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rsetA.getString(1)),pFontHeader1);
                                            Net12 = Net12 + rsetA.getDouble(1);
                                            table.addCell(phrase);
                                        }
                                    }
...
docPdf.close();  com.afrisoftech.lib.PDFRenderer.renderPDF(tempFile);
    ...
    public java.lang.Object[] getListofStaffNos() {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT staff_no FROM posting WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND description = 'P.A.Y.E' ORDER BY staff_no");
