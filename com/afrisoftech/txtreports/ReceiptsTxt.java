...
public class ReceiptsTxt implements java.lang.Runnable {
    ...
    private void writeReport(java.io.RandomAccessFile txtRandomAccessFile){
        ...
        try{
            ...
            java.sql.ResultSet rset51 = st51.executeQuery("select distinct patient_no,dealer from ac_cash_collection where receipt_no = '"+Receipt+"'");
            
            java.sql.ResultSet rset5 = st5.executeQuery("SELECT INITCAP(description),"
                    + "SUM(quantity),SUM(debit-credit),round(SUM(debit/quantity)) FROM "
                    + " ac_cash_collection WHERE receipt_no = '"+Receipt+"' "
                    + " GROUP BY description ORDER BY SUM(debit) DESC");
            ...
            while(rset51.next()){
                MNo = rset51.getString(1);
                Name = rset51.getString(2);
            }
