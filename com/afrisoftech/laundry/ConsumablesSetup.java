...
public class ConsumablesSetup extends javax.swing.JInternalFrame {
     ...
     */
    public ConsumablesSetup(Connection passedConn) {
        setVisible(true);
        conn=passedConn;
        initComponents();
        refreshCbx();
    ...
    private void initComponents() {
        ...
        txtTagNo.setBackground(new java.awt.Color(255, 255, 255));
    ...
    private void cbxItemNameItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cbxItemNameItemStateChanged
     try {
            ...
            java.sql.ResultSet rset = stmt.executeQuery("SELECT DISTINCT item_code   FROM st_stock_item where sub_cat_code ilike '%LAUNDRY DETERGENTS%' and  description ilike '"+cbxItemName.getSelectedItem()+"'order by 1 asc limit 1");
              while (rset.next()) {
              txtTagNo.setText(rset.getString(1));
              }
    ...
    public void populateTable(){
        try {
            ...
            String sql="SELECT DISTINCT item_code, description   FROM st_stock_item where sub_cat_code ilike '%LAUNDRY DETERGENTS%' and item_code IN (SELECT item_code FROM laundryconsumables) order by 1 asc";
...
ResultSet.CONCUR_UPDATABLE);
            ...
            int counter=0;
            while ( rs.next() ) {
                batch[counter][0]=rs.getString(2);
                brandTagsTable.add(rs.getString(1));
                counter++;
            }
            RefreshClientsTable();
    ...
    public void refreshCbx(){
        try {
          ...
          String sql2="SELECT DISTINCT description,item_code   FROM st_stock_item where sub_cat_code ilike '%LAUNDRY DETERGENTS%' and item_code NOT IN (SELECT item_code FROM laundryconsumables) order by 1 asc";
            Statement stmt2 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            ResultSet rs2 = stmt2.executeQuery(sql2 );
            cbxItemName.removeAllItems();
            brandTags.clear();
            while(rs2.next()){
                cbxItemName.addItem(rs2.getString(1));
                brandTags.add(rs2.getString(2));
            }
        } catch (SQLException ex) {
            Logger.getLogger(ConsumablesSetup.class.getName()).log(Level.SEVERE, null, ex);
        }
