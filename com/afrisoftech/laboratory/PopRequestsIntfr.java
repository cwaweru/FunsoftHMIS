 ...
 * @author  root
 */
public class PopRequestsIntfr extends javax.swing.JInternalFrame implements java.lang.Runnable {
    ...
    public PopRequestsIntfr(java.sql.Connection connDb, org.netbeans.lib.sql.pool.PooledConnectionSource pconnDB) {
            ...
            if (Boolean.valueOf(jTable1.getModel().getValueAt(i, 9).toString()) == java.lang.Boolean.TRUE) {
                ...
                if (jTable1.getValueAt(i, 9) != null) {
                    ...
                    java.sql.ResultSet rsetTable1 = stmtTable1.executeQuery("SELECT paid FROM hp_patient_billing " +
                            " WHERE paid = true AND trans_date > (select current_timestamp(0)::date -2) AND patient_no = '" + this.jTextField9.getText() + "' " +
        ...
        if (this.jCheckBox1.isSelected() && this.jCheckBox31.isSelected()) {
            ...
            if (this.jCheckBox1.isSelected() && this.jCheckBox4.isSelected()) {
                if (jTextField113.getCaretPosition() < 3) {
                    ...
                    jSearchTable2.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "SELECT patient_no, (upper(second_name||' '||first_name||' '||last_name)) as name, year_of_birth, residence from hp_patient_register where name ILIKE '%" + jTextField113.getText() + "%' AND last_visit >= (select current_timestamp(0)::date -2) order by name LIMIT 10"));
...
jEditorPane2.setText("");
        ...
        try {
            ...
            java.sql.ResultSet rset1 = stmt1.executeQuery("select category,pay_mode,residence,((select current_timestamp(0)::date -2)-year_of_birth::date)/365 from hp_patient_register where patient_no  ='" + jTextField9.getText() + "'");
            while (rset1.next()) {
    ...
    private void populateTable1(java.lang.String patient_no) {
        ...
        try {
            ...
            java.sql.ResultSet rsetTable1 = stmtTable1.executeQuery("select trans_date,patient_no,patient_name,payment_mode,service,quantity,amount/quantity,inpatient_no,user_name,paid as seen,visit_id,trans_date,amount from hp_patient_billing pb, pb_operating_parameters op WHERE pb.service = op.service_type AND op.category ILIKE 'Pop' AND collected = false AND pb.trans_date > (select current_timestamp(0)::date -2) AND patient_no = '" + jTextField9.getText() + "' ORDER BY trans_date asc");
...
jTable1.setValueAt(rsetTable1.getObject(13), i, 12);
    ...
    public void runGetListThread() {
        ...
        try {
            ...
            java.sql.ResultSet rsetTable11 = stmtTable11.executeQuery("select distinct count(patient_no) from hp_patient_billing pb, pb_operating_parameters op WHERE pb.service = op.service_type AND op.category ILIKE 'Pop' AND collected = false AND trans_date > (select current_timestamp(0)::date -2)");
        ...
        if (patNo > 5) {
            ...
            this.jLabel1.setText("Queue Long '" + patNo + "' Patients Are Waiting For Pop");
