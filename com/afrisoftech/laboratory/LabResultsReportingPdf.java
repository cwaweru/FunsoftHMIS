...
public class LabResultsReportingPdf implements java.lang.Runnable {
...

                            {
                                ...
                                try {
                                    ...
                                    for (int k = 0; k < listOfLabs.length; k++) {
                                        System.out.println(""
                                                + "SELECT  request_id,  typeof_test, parameter, result,\n"
                                                + "       units, upper_limit, lower_limit,  out_come||''||  comments as outcome,\n"
                                                + "       lab_no,time_taken, input_date as posting_time, \n"
                                                + "        lab_manager, pathologist, user_name, spec_time,  code, result_no\n"
                                                + "          FROM hp_lab_results where patient_no='" + patientNO + "' and date='" + Qdate + "' "
                                                + "   and typeof_test in "
                                                + "(select distinct request from clerking_requests where request_category='" + listOfLabs[k] + "')  ");
        ...
        try {
            ...
            System.out.println(
                  "select distinct request_category FROM clerking_requests where\n"
                    + "   request::text in (SELECT distinct typeof_test::text FROM hp_lab_results where"
                    + " patient_no='" + patientNO + "' and trans_date='" + date + "' )order by 1 asc");
            // java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT patient_no FROM hp_patient_card WHERE date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND payment_mode = 'Scheme' AND isurer = '"+memNo+"' order by patient_no");
