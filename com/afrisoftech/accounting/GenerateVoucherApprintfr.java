 ...
 * @author root
 */
public class GenerateVoucherApprintfr extends javax.swing.JInternalFrame {
    ...
    public GenerateVoucherApprintfr(java.sql.Connection connDb, org.netbeans.lib.sql.pool.PooledConnectionSource pconnDB) {
    ...
    private void initComponents() {
        ...
        */
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Invoice Date", "Invoice/Imprest No.", "Transaction Type", "Item", "Amount", "Amt Approved", "Approve ?", "Invoice/Imprest No."
            }
        ) {
        ...
        jButton5.setText("Generated Vouchers");
...
"Where   ac_accounts_payable.dealer ILike '%" + jTextField113.getText() + "%' And      ac_accounts_payable.voucher_no not like'VC%' And   ac_accounts_payable.approved = False order by 2");
...
"  ac_bills.final_voucher_no not like'VC%' And " +
...
//select date_due,invoice_no, transaction_type,item,(credit-debit) AS AMOUNT FROM ac_accounts_payable where  dealer ='SURGILINKS LIMITED' and invoice_no in (select invoice_no from ac_accounts_payable group by invoice_no having sum(credit-debit) != 0 )
    ...
    private void jSearchTable2MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jSearchTable2MouseClicked
        ...
        dealerNameTxt.setText(jSearchTable2.getValueAt(jSearchTable2.getSelectedRow(), 1).toString());
               if (supplierChkbx.isSelected()) {
                   System.out.println("select date_due,invoice_no, transaction_type,item,"
                    + "(credit-debit) AS AMOUNT, 0.00 as approved_amount, false as approve,invoice_no FROM "
                    + "ac_accounts_payable where approved=false "
                    + " and voucher_no not ilike 'VC%'  "
            ...
            jTable1.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "select date_due,invoice_no, transaction_type,item,"
                    ...
                    + "and date BETWEEN '" + datePicker1.getDate() + "' AND '" + datePicker2.getDate() + "' and dealer ='" + dealerNameTxt.getText() + "' and invoice_no in (select invoice_no from ac_accounts_payable group by invoice_no having sum(credit-debit) != 0 ) order by invoice_no"));
                      ...
                      if(correct_name.contains("'")){
                       ...
                       System.out.println("SELECT date_deu as date_due,voucher_no as imprest_no,description,reason,credit as amount, 0.00 as approved_amount, false as approve,final_voucher_no "
                    + " FROM ac_bills WHERE approved = false"
                    + "and final_voucher_no not ilike 'VC%' "
                        ...
                        if (jTable1.getModel().getValueAt(i, 6).toString().equals("true")) {
                            ...
                            if (supplierChkbx.isSelected() == true) {
                                ...
                                java.sql.PreparedStatement pstmt = connectDB.prepareStatement(""
                                        + "UPDATE ac_accounts_payable set  journal_no='" + jTable1.getModel().getValueAt(i, 1) + "',invoice_no='" + jTable1.getModel().getValueAt(i, 7) + "',"
                                        + "cash_point = '" + requNo + "',voucher_no = '" + requNo + "' WHERE "
                                        + "invoice_no = '" + jTable1.getModel().getValueAt(i, 1).toString() + "' and dealer ILIKE '" + this.dealerNameTxt.getText() + "'");

                                pstmt.executeUpdate();
                            } else if (imprestChkbx.isSelected() == true) {
                                java.sql.PreparedStatement pstmt = connectDB.prepareStatement("UPDATE ac_bills set   final_voucher_no = '" + requNo + "',voucher_no = '" + jTable1.getModel().getValueAt(i, 7).toString() + "' WHERE"
                                        + " voucher_no = '" + jTable1.getModel().getValueAt(i, 1).toString() + "' and dealer ILIKE '" + this.dealerNameTxt.getText() + "'");
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        com.afrisoftech.hospital.HospitalMain.generatedVouchersitemmnit.doClick();
