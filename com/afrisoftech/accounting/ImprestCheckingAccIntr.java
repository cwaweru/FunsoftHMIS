 ...
 * @author SPLgithinji
 */
public class ImprestCheckingAccIntr extends javax.swing.JInternalFrame {
...
java.sql.Connection connectDB = null;
 ...
 String desc = null;
     ...
     */
public ImprestCheckingAccIntr(java.sql.Connection connDb, org.netbeans.lib.sql.pool.PooledConnectionSource pconnDB) {
    ...
    private void initComponents() {
        ...
        */
        ...
        unsurrenderedAmountTxt.setFont(new java.awt.Font("Dialog", 1, 12)); // NOI18N
        ...
        totalAmountIssuedTxt.setFont(new java.awt.Font("Dialog", 1, 12)); // NOI18N
        ...
        jLabel3.setText("IMPREST SURRENDERED CHECKING");
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 0;
        gridBagConstraints.gridy = 0;
        gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
    ...
    private void imprestSearchTxtCaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_imprestSearchTxtCaretUpdate
        // TODO add your handling code here:
        if(this.imprestSearchTxt.getCaretPosition() > 2){
        ApprovalTbl.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "select distinct staff_veh, payee,voucher_no from ac_petty_cash where (voucher_no !='' and (voucher_no ilike '%PETT%') or (voucher_no ilike '%S_IMPREST%' )or (voucher_no ilike '%T_IMPREST%')) "
                + "and (payee ILIKE '%" + imprestSearchTxt.getText().toString() + "%' or staff_veh ILIKE '%" + imprestSearchTxt.getText().toString() + "%')  and status != 'Payment Done' and approved_surrender = false and checked_surrender = false GROUP BY staff_veh, payee,voucher_no having sum(debit-credit) >= 0 or sum(credit-debit)>0 ORDER BY 1"));
        }
            ...
            try{
                ...
                try{
                    ...
                    if(java.lang.Double.valueOf(totalAmounttxt.getText().toString().length())>0.0){
                     ...
                     else if(unstockableItemsRdo.isSelected()){
                    ...
                    javax.swing.JOptionPane.showMessageDialog(this, "Checking Done Successfully","Comfirmation Message",javax.swing.JOptionPane.INFORMATION_MESSAGE);
                    
                    ApprovalTbl.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "select distinct staff_veh, payee,voucher_no from ac_petty_cash where "
                            + "(voucher_no !='' and (voucher_no ilike '%PETT%') or (voucher_no ilike '%S_IMPREST%' )or (voucher_no ilike '%T_IMPREST%')) "
                            + "and status != 'Payment Done' and approved_surrender = false and checked_surrender = false GROUP BY staff_veh, payee,voucher_no having (sum(debit-credit) >= 0 or sum(credit-debit)>0) and sum(credit)>0 ORDER BY 1"));
                    for (int k = 0; k < ImprestTable.getRowCount(); k++ ) {
                        for (int r = 0; r < ImprestTable.getColumnCount(); r++ ) {
                            ImprestTable.getModel().setValueAt(null,k,r);
                        }
...
//select distinct staff_veh, payee,voucher_no from ac_petty_cash where (voucher_no !='' and (voucher_no ilike '%PETT%') or (voucher_no ilike '%S_IMPREST%' )or (voucher_no ilike '%T_IMPREST%')) and status != 'Payment Done' and approved_surrender = false GROUP BY staff_veh, payee,voucher_no having (sum(debit-credit) >= 0 or sum(credit-debit)>0) and sum(credit)>0 ORDER BY 1
                ApprovalTbl.setModel(com.afrisoftech.dbadmin.TableModel.createTableVectors(connectDB, "select distinct staff_veh, payee,voucher_no from ac_petty_cash where (voucher_no !='' and (voucher_no ilike '%PETT%') or (voucher_no ilike '%S_IMPREST%' )or (voucher_no ilike '%T_IMPREST%')) and status != 'Payment Done' and approved_surrender = false and checked_surrender = false GROUP BY staff_veh, payee,voucher_no having (sum(debit-credit) >=0 or sum(credit-debit) > 0) and sum(credit)>0 ORDER BY 1"));
                invoiceNoLbl.setText("Invoice No:");
                unsurrenderedAmtLbl.setText("Unsurrendered Amount");
            }
