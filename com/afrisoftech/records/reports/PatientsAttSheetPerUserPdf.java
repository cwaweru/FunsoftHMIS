...
public class PatientsAttSheetPerUserPdf implements java.lang.Runnable {
    ...
    public void PatientsAttSheetPerUserPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate, java.lang.String user) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {7, 12, 30, 8, 8, 20, 16, 12};
                        ...
                        try {
                            ...
                            phrase = new Phrase("Patient List Registered By : [ "+userName.toUpperCase()+" ]     Period : " + dateFormat.format(endDate11) + " ------ " + dateFormat.format(endDate1), pFontHeader);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st11.executeQuery("select count(distinct(patient_no)) from hp_patient_visit WHERE date BETWEEN '" + beginDate + "' AND '" + endDate + "' and transaction_type ilike 'regis%' AND user_name ILIKE '" + userName + "'");

                            for (int i = 0; i < listofAct.length; i++) {
                                System.out.println("item" + listofAct[i]);
                                java.sql.Statement st = connectDB.createStatement();

                                java.sql.PreparedStatement st1 = connectDB.prepareStatement("select distinct pr.patient_no,initcap(pb.name),ROUND(pb.age), pb.gender,initcap(pb.services),initcap(pr.residence),pr.date::date from hp_patient_register pr,hp_patient_visit pb WHERE pb.date::date BETWEEN '" + beginDate + "' AND '" + endDate + "' and pr.patient_no = ? and pb.patient_no = pr.patient_no  AND user_name ILIKE '" + userName + "' ORDER BY pr.date::date desc limit 1");
...
docPdf.close();  com.afrisoftech.lib.PDFRenderer.renderPDF(tempFile);
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT patient_no,date FROM hp_patient_visit where transaction_type ilike 'registration%' and date BETWEEN '" + beginDate + "' AND '" + endDate + "'  AND user_name ILIKE '" + userName + "' order by date");
