...
public class MonthlyOPWorkLoadPdf_old implements java.lang.Runnable {
...
//Pharmacy & Farewell
                            ...
                            rsp113 = stp113.executeQuery("SELECT COUNT(patient_no) FROM ac_ledger"
                                    + " WHERE date::date BETWEEN '" + beginDate + "' AND '" + endDate + "'  AND "
                                    + "(description ILIKE '%Xray%' OR description  ILIKE '%Ultra%' OR description  ILIKE '%x-ray%' OR description  ILIKE '%radiology%' OR description  ILIKE '%imaging%' ) AND UPPER(service_type) IN  (SELECT UPPER(service_type) FROM pb_operating_parameters  WHERE UPPER(service_classification) = 'ULTRASOUND' ) ");
                            while (rsp113.next()) {
