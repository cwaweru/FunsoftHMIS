...
public class PatientsAttSheetByDepPdf implements java.lang.Runnable {
    ...
    public void PatientsAttSheetByDepPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate, java.lang.String dept) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {7, 12, 20, 6, 7, 2, 15, 12,10,10,10};
                        ...
                        table.addCell(phrase);
                        phrase = new Phrase("Mariral status", pFontHeader);
                        table.addCell(phrase);
                        phrase = new Phrase("Date - First \n Attendance", pFontHeader);
                        table.addCell(phrase);
                        phrase = new Phrase("Time Registered", pFontHeader);
                        table.addCell(phrase);
                        phrase = new Phrase("Time Seen", pFontHeader);
                        table.addCell(phrase);
                        phrase = new Phrase("Time Taken", pFontHeader);
                        table.addCell(phrase);
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                st = connectDB.prepareStatement("SELECT pv.input_date::TIME(0), pr.curr_date::TIME(0),pr.curr_date::TIME(0)-pv.input_date::TIME(0) FROM hp_patient_visit pv,pb_doctors_request pr WHERE pr.curr_date::date BETWEEN '" + beginDate + "' AND '" + endDate + "' and pr.patient_no = ? AND pr.patient_no = pv.patient_no AND pr.clinic ILIKE '" + departMent + "%' ORDER BY 1 ASC LIMIT 1");
                                st.setString(1, listofAct[i].toString());
                                rset1 = st.executeQuery();
                                while (rset1.next()) {
                                    regTime = dbObject.getDBObject(rset1.getObject(1), "-");
                                    timeSeen = dbObject.getDBObject(rset1.getObject(2), "-");
                                    timeTaken = rset1.getString(3);
                                }
                                ...
                                while (rset.next()) {
                                    ...
                                    phrase = new Phrase(timeTaken, pFontHeader1);
