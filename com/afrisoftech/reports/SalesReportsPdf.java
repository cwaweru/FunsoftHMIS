...
public class SalesReportsPdf implements java.lang.Runnable {
    ...
    public void SalesReportsPdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate,java.lang.String cashPoint, java.lang.String glAccount) {
                        ...
                        try {
                            ...
                            for (int k = 0; k < listofAct1.length; k++) {
                                ...
                                for (int i = 0; i < listofAct.length; i++) {//AND st.transfer_price > 0
                                    ...
                                    java.sql.PreparedStatement stbx = connectDB.prepareStatement("select sum(credit-debit) FROM ac_ledger WHERE activity_code = '"+gAccount+"' and service_type ilike ? AND date BETWEEN '"+beginDate+"' AND '"+endDate+"' GROUP BY service_type");
                                    
//  java.sql.PreparedStatement st1 = connectDB.prepareStatement("select ph.description,sum(ph.quantity) as quantity,avg(st.transfer_price)::numeric(10,2) as bprice,sum((ph.quantity * st.transfer_price)::numeric(10,2)) as cost, avg(st.selling_price)::numeric(10,2)as sprice,sum(st.selling_price *ph.quantity)::numeric(10,2) as amt,sum(st.selling_price *ph.quantity) - sum(ph.quantity * st.transfer_price)::numeric(10,2),CASE WHEN (sum(transfer_price) = 0) THEN 0.00 ELSE (avg(st.selling_price)/avg(st.transfer_price))::numeric(10,2) END AS markup from hp_pharmacy ph,st_stock_prices st WHERE (st.department ilike 'PHAr%' OR st.department ilike 'IP Pharmacy%') and ph.description = ? AND ph.date_prescribed BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ph.description = st.product GROUP BY ph.description");
                                    ...
                                    while (rset.next()) {
                                        ...
                                        */
                                            ...
                                            osBalance2 = osBalance2 + rsetbx.getDouble(1);
                                ...
                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(osBalance23)), pFontHeader);
    ...
    public java.lang.Object[] getListofActivities(java.lang.Object Cate) {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT ph.service_type FROM ac_ledger ph,st_stock_prices st where ph.date BETWEEN '"+beginDate+"' AND '"+endDate+"'  AND ph.activity_code = '"+gAccount+"' and ph.service_type = st.product and st.category ilike '"+Cate+"%' ORDER BY ph.service_type");
