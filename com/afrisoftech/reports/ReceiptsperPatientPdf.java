                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset3 = st3.executeQuery("SELECT DISTINCT hospital_name,postal_code||' '||box_no,main_telno||' '||other_telno,initcap(street),main_faxno,email,website,room_no from pb_hospitalprofile");
                            java.sql.ResultSet rset1 = st1.executeQuery("SELECT DISTINCT dealer,receipt_no,date from ac_cash_collection where receipt_no = '"+memNo+"' AND transaction_type = 'Receipts' order by date");
                            ...
                            java.sql.ResultSet rs = st.executeQuery("SELECT DISTINCT receipt_no from ac_cash_collection where receipt_no = '"+memNo+"'");
                            java.sql.ResultSet rset6 = st6.executeQuery("SELECT DISTINCT CASE WHEN (payment_mode = 'Cash') THEN 'Cash' ELSE card_name||' '||card_no END AS payment_mode, user_name from ac_cash_collection where receipt_no = '"+memNo+"' AND transaction_type = 'Receipts'");
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                java.sql.ResultSet rset5 = st5.executeQuery("select pc.main_service, pc.dosage,SUM(pc.credit) from hp_patient_card pc WHERE pc.reference ='"+listofAct[i].toString()+"' GROUP BY pc.main_service, pc.dosage");// group by pc.main_service");
