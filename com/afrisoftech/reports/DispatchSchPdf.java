...
import static com.afrisoftech.reports.ShiftReconPdf.connectDB;
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {15, 15, 30, 20, 10, 15, 15, 15, 15, 15, 8};
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select date,journal_no, initcap(item),invoice_no ||' '||receipt_no,debit,debit,null as boolean, (select distinct f_name || ' '|| l_name from secure_menu_access where login_name = ac_debtors.user_name limit 1), (CASE WHEN claim_no is null THEN (select claim_no from hp_patient_discharge where inv_no = ac_debtors.invoice_no) ELSE claim_no END) as claim_no from ac_debtors where dispatch_no = '" + memNo + "' and invoice_no in (select invoice_no from ac_debtors where dispatch_no = '"+memNo+"' group by invoice_no having sum(debit-credit) > 0)  order by 3");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                            ...
                            table.addCell(phrase);
                            phrase = new Phrase("Rebate Type", pFontHeader1);
                            table.addCell(phrase);
                            ...
                            while (rset1.next()) {
                                ...
                                table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(String.valueOf(x), pFontHeader);
                                ...
                                java.sql.PreparedStatement pstmt = connectDB.prepareStatement("SELECT distinct visit_id from hp_patient_card where invoice_no = ? order by 1 desc limit 1");
                                pstmt.setString(1, rset1.getString(4));
                                java.sql.ResultSet rsets = pstmt.executeQuery();
                                String wardName = "-";
                                while (rsets.next()) {
                                    System.out.println("Printing Visit ID [" + rsets.getString(1) + "]");
                                    java.sql.PreparedStatement pstmts = connectDB.prepareStatement("select ward from hp_admission where visit_id = ?");
                                    pstmts.setString(1, rsets.getString(1));
                                    java.sql.ResultSet rsetss = pstmts.executeQuery();
                                    while (rsetss.next()) {
                                        System.out.println("Printing Ward Name : [" + rsetss.getString(1) + "]");
                                        wardName = rsetss.getString(1);
...
//                            table.addCell(phrase);
                            ...
                            while (rset412.next()) {
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Inv. Count : " + dbObject.getDBObject(x, "-"), pFontHeader1);
                                table.addCell(phrase);
                            }
                            table.getDefaultCell().setColspan(1);
                            ...
                            table.addCell(phrase);
                            table.getDefaultCell().setColspan(5);
                            phrase = new Phrase("CHECKED BY: ____________________________________", pFontHeader1);
                            table.addCell(phrase);
                            table.getDefaultCell().setColspan(11);
                            phrase = new Phrase(" ");
                            table.addCell(phrase);
