...
public class GroupedShiftSurrenderPdf implements java.lang.Runnable {
    ...
    public void GroupedShiftSurrenderPdf(java.sql.Connection connDb, java.lang.String combox, java.lang.String userName) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("CASHIER'S SHIFT REPOR SUMMARY by DEPARTMENTT", pFontHeaderB);
                            table.addCell(phrase);
...
//                                java.lang.Object[] listofAct1 = this.getListofActivities1(listofAct[i]);
                            ...
                            // table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Shift Breakdown Summary by Department".toUpperCase(), pFontHeaderB);
                        ...
                        //    for (int x = 0; x < listofAct11.length; x++) {
                                ...
                                java.sql.ResultSet rsets = sts.executeQuery("select (SELECT country_name from pb_activity where pb_activity.code = ac_cash_collection.activity_code),(SELECT depart_name  FROM pb_main_department where department_code in (SELECT country_name from pb_activity where pb_activity.code = ac_cash_collection.activity_code) ) ,SUM(debit-credit) from ac_cash_collection where shift_no = '"+memNo+"'  GROUP BY 1, 2 ORDER BY 1");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
...
////                                java.sql.ResultSet rsets = sts.executeQuery("select DISTINCT upper(activity_code),SUM(debit-credit),price_category from ac_cash_collection where shift_no = '" + memNo + "'  AND activity_code = '" + listofAct11[x].toString() + "' GROUP BY activity_code,price_category ORDER BY price_category ");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
                            ...
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            table.getDefaultCell().setColspan(2);
                            phrase = new Phrase("DEPARTMENT CODE", pFontHeaderB);

                            table.addCell(phrase);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            table.getDefaultCell().setColspan(4);
                            phrase = new Phrase("DEPARTMENT NAME", pFontHeaderB);
