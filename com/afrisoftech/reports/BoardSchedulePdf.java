...
public class BoardSchedulePdf implements java.lang.Runnable {
    ...
    public void BoardSchedulePdf(java.sql.Connection connDb, java.lang.String scheduleNumber, boolean printPreview) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("Board Member Payment Schedule  - Page: ", pFontHeader), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
...
//////////select dealer,chq_date,cheque_no,initcap(payment_mode), account_no, account_name,sum(credit) as amount  from ac_cash_book  where schedule_no = '" + scheduleNo + "' group by dealer,chq_date,cheque_no,initcap(payment_mode), account_no, account_name
                            ...
                            table.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                            Phrase phrase = new Phrase("PAYMENTS SCHEDULE NO: [" + scheduleNo + "] of DATE : [" + com.afrisoftech.lib.ServerTime.serverDate(connectDB)
                                    + "]\n"
                            ...
                            table.addCell(phrase);
                            phrase = new Phrase("Board Member Name", pFontHeader1);
                            ...
                            if (rtgsCount > 0) {
                                ...
                                String dealer = null;
                                String code = null;
                                java.sql.Statement stVoucher = connectDB.createStatement();
                                java.sql.ResultSet rsVoucher = stVoucher.executeQuery("select dealer,dealer_code from ac_other_payments where schedule_no='" + scheduleNo + "'");       
                                 while (rsVoucher.next()) {
                                  dealer = rsVoucher.getString(1);
                                  code = rsVoucher.getString(2);
                                 }
                                java.sql.Statement stDetails = connectDB.createStatement();
                                java.sql.ResultSet rsDetails = stDetails.executeQuery("select dealer, (select bank_account_no from master_file where id_no = ac_other_payments.dealer_code ) as bank, "
                                        + "(select bank from master_file where id_no = ac_other_payments.dealer_code) as bank_account,(select branch from master_file where id_no = ac_other_payments.dealer_code) as branch,"
                                        + "dealer_code as ID_NO, credit as Net_Amount from ac_other_payments where schedule_no='" + scheduleNo + "'");       
                                table.getDefaultCell().setColspan(1);
                                table.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                                int index = 1;
