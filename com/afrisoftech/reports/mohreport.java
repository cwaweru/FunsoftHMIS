...
public class mohreport implements java.lang.Runnable {
    ...
    public void mohreport(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {3, 15, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10};
                        ...
                        phrase = new Phrase("UHC Reg. Fee", pFontHeader);
                        table.addCell(phrase);
                        ...
                        phrase = new Phrase("UHC ", pFontHeader);
                        table.addCell(phrase);

                        phrase = new Phrase("Free Elderly ", pFontHeader);
                        table.addCell(phrase);
                        ...
                        try {
                            ...
                            double Tgross = 0.00;
                            double freeelderlytotaldebts = 0.00;
                            double uhctotaldebts = 0.00;
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                double uhcdebts = 0.00;

                                double freeelderlydebts = 0.00;

                                java.sql.PreparedStatement psetwn1x = connectDB.prepareStatement("select code from pb_activity where department ilike 'IEXE'");
                                java.sql.ResultSet rsetwn1x = psetwn1x.executeQuery();
                                while (rsetwn1x.next()) {
                                    java.sql.PreparedStatement psetwn2 = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection WHERE date = '" + listofAct[i] + "' AND activity_code ILIKE '" + rsetwn1x.getString(1) + "'");
                                ...
                                Double debtors = 0.00;
                                java.sql.PreparedStatement psetwn1dsfv = connectDB.prepareStatement("select  distinct ac.invoice_no from ac_debtors ac,hp_patient_card hp WHERE ac.date = '" + listofAct[i] + "'   \n"
                                        + " AND ac.debit > 0 and ac.invoice_no=hp.invoice_no and hp.debit>0 order by 1");
                                java.sql.ResultSet rsetwn1dsfv = psetwn1dsfv.executeQuery();
                                while (rsetwn1dsfv.next()) {

                                    Double debtortotal = 0.00;
                                    java.sql.PreparedStatement v = connectDB.prepareStatement("  select sum(debit-credit) from ac_debtors WHERE invoice_no='" + rsetwn1dsfv.getString(1) + "'");
                                    java.sql.ResultSet p = v.executeQuery();
                                    while (p.next()) {
                                        debtortotal = p.getDouble(1);
                                    }
                                    Double billtotal = 0.00;
                                    java.sql.PreparedStatement v1 = connectDB.prepareStatement("SELECT   sum(debit- credit) FROM public.hp_patient_card where   service not ilike '%invoice%' and invoice_no='" + rsetwn1dsfv.getString(1) + "'");
                                    java.sql.ResultSet p1 = v1.executeQuery();
                                    while (p1.next()) {
                                        billtotal = p1.getDouble(1);
                                    }
                                    System.err.println(debtortotal + " " + billtotal);
                                    System.out.println(debtortotal - billtotal);
                                    if (debtortotal - billtotal == 0) {
                                        debtors = debtors + debtortotal;
                                    }
                                    
                                    

//                                    java.sql.PreparedStatement psetwn1dssa = connectDB.prepareStatement("select sum(debit-credit) from ac_debtors WHERE invoice_no= '" + rsetwn1dsfv.getString(1) + "' ");
//                                    java.sql.ResultSet rsetwn1dssa = psetwn1dssa.executeQuery();
//                                    while (rsetwn1dssa.next()) {
//                                        debtors = debtors + rsetwn1dssa.getDouble(1);
//                                    }
                                }
                                ...
                                while (rsethnh.next()) {
                                    ...
                                    java.sql.PreparedStatement psetwn1dssj = connectDB.prepareStatement("select  distinct ac.invoice_no from ac_debtors ac,hp_patient_card hp WHERE ac.date = '" + listofAct[i] + "'  AND ac.payee ilike 'FREE ELDERLY H.S' \n"
                                            + " AND ac.debit > 0 and ac.invoice_no=hp.invoice_no and hp.debit>0 order by 1");
                                    java.sql.ResultSet rsetwn1dssj = psetwn1dssj.executeQuery();
                                    while (rsetwn1dssj.next()) {
                                        Double debtortotal = 0.00;
                                        java.sql.PreparedStatement v = connectDB.prepareStatement("  select sum(debit-credit) from ac_debtors WHERE invoice_no='" + rsetwn1dssj.getString(1) + "'");
                                        java.sql.ResultSet p = v.executeQuery();
                                        while (p.next()) {
                                            debtortotal = p.getDouble(1);
                                        }
                                        Double billtotal = 0.00;
                                        System.err.println("select sum(debit-credit) from ac_debtors WHERE invoice_no='" + rsetwn1dssj.getString(1) + "'");
                                        System.err.println("SELECT   sum(debit- credit) FROM public.hp_patient_card where   service not ilike '%invoice%' and invoice_no='" + rsetwn1dssj.getString(1) + "'");
                                        java.sql.PreparedStatement v1 = connectDB.prepareStatement("SELECT   sum(debit- credit) FROM public.hp_patient_card where   service not ilike '%invoice%' and invoice_no='" + rsetwn1dssj.getString(1) + "'");
                                        java.sql.ResultSet p1 = v1.executeQuery();
                                        while (p1.next()) {
                                            billtotal = p1.getDouble(1);
                                        }
                                        ...
                                        System.out.println(debtortotal - billtotal);
                                        if (debtortotal - billtotal == 0) {
                                            java.sql.PreparedStatement psetwn1dss = connectDB.prepareStatement("select sum(debit-credit) from ac_debtors WHERE invoice_no='" + rsetwn1dssj.getString(1) + "'");
                                            java.sql.ResultSet rsetwn1dss = psetwn1dss.executeQuery();
                                            while (rsetwn1dss.next()) {
                                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                                freeelderlydebts = freeelderlydebts + rsetwn1dss.getDouble(1);
                                                freeelderlytotaldebts = freeelderlytotaldebts + rsetwn1dss.getDouble(1);
                                            }
                                        }
                                    }
                                    ...
                                    //--------------------
                                    java.sql.PreparedStatement psetwn1dsf = connectDB.prepareStatement("select  distinct ac.invoice_no from ac_debtors ac,hp_patient_card hp WHERE ac.date = '" + listofAct[i] + "'  AND ac.payee ilike 'UNIVERSAL HEALTH CARE' \n"
                                            + " AND ac.debit > 0 and ac.invoice_no=hp.invoice_no and hp.debit>0 order by 1");
                                    java.sql.ResultSet rsetwn1dsf = psetwn1dsf.executeQuery();
                                    while (rsetwn1dsf.next()) {
                                        Double debtortotal = 0.00;
                                        java.sql.PreparedStatement v = connectDB.prepareStatement("  select sum(debit-credit) from ac_debtors WHERE invoice_no='" + rsetwn1dsf.getString(1) + "'");
                                        java.sql.ResultSet p = v.executeQuery();
                                        while (p.next()) {
                                            debtortotal = p.getDouble(1);
                                        }
                                        Double billtotal = 0.00;
                                        System.err.println("select sum(debit-credit) from ac_debtors WHERE invoice_no='" + rsetwn1dsf.getString(1) + "'");
                                        System.err.println("SELECT   sum(debit- credit) FROM public.hp_patient_card where   service not ilike '%invoice%' and invoice_no='" + rsetwn1dsf.getString(1) + "'");
                                        java.sql.PreparedStatement v1 = connectDB.prepareStatement("SELECT   sum(debit- credit) FROM public.hp_patient_card where   service not ilike '%invoice%' and invoice_no='" + rsetwn1dsf.getString(1) + "'");
                                        java.sql.ResultSet p1 = v1.executeQuery();
                                        while (p1.next()) {
                                            billtotal = p1.getDouble(1);
                                        }
                                        ...
                                        System.out.println(debtortotal - billtotal);
                                        if (debtortotal - billtotal == 0) {
                                            System.err.println("it is equal..................................");
                                            java.sql.PreparedStatement psetwn1ds = connectDB.prepareStatement("select sum(debit-credit) from ac_debtors WHERE invoice_no='" + rsetwn1dsf.getString(1) + "'");
                                            java.sql.ResultSet rsetwn1ds = psetwn1ds.executeQuery();
                                            while (rsetwn1ds.next()) {
                                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                                uhcdebts = uhcdebts + rsetwn1ds.getDouble(1);
                                                uhctotaldebts = uhctotaldebts + rsetwn1ds.getDouble(1);;
                                    ...
                                    java.sql.PreparedStatement psetwn4 = connectDB.prepareStatement("select sum(debit-credit) from ac_debtors WHERE date = '" + listofAct[i] + "'  AND activity_code != '" + nhifs + "' AND (payee not ilike 'UNIVERSAL%' AND payee not ilike 'FREE ELDER%') AND debit > 0");
                                ...
                                debtors=othdebts+nhifdebts+uhcdebts+freeelderlydebts;
                                System.err.println("Date "+ listofAct[i]);
                                System.err.println("Cash "+ janu);
                                System.err.println("nhif "+ nhifdebts);
                                System.err.println("uhc "+ uhcdebts);
                                System.err.println("elderly "+ freeelderlydebts);
                                System.err.println("Others "+ othdebts);
                                System.err.println("Total debtors "+ debtors);
                                janu = janu + debtors - exedebts;
                                System.err.println("Sales "+ janu);

                                System.out.println("The dated.................................." + listofAct[i]);

                                System.out.println("Amount.................................." + janu);
                                System.out.println("Total.................................." + Tsales);
...
//                                double exemptionDepartmet = 0.00;
//                                java.sql.PreparedStatement exemp = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking'  and (activity ilike '%ward%' or activity ilike '%amenity%') ");
//                                java.sql.ResultSet exempr = exemp.executeQuery();
//
//                                while (exempr.next()) {
//                                    exemptionDepartmet = exempr.getDouble(1);
//
//                                }

                                java.sql.PreparedStatement pset1t = connectDB.prepareStatement("select code from pb_activity where department ilike 'IPP' ");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                java.sql.ResultSet rset1t = pset1t.executeQuery();
                                while (rset1t.next()) {

                                    java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection WHERE date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code ilike '" + rset1t.getString(1) + "' ");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                //currentIPPTotal = currentIPPTotal + exemptionDepartmet;
                                //}
                                ...
                                java.sql.PreparedStatement psetj1 = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code=code and uhc_tag  ilike 'UHC' ");
...
//                                exemp = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking'  and (activity  ilike 'XRAY%' OR activity  ilike 'X-RAY%') ");
//                                exempr = exemp.executeQuery();
//
//                                while (exempr.next()) {
//                                    exemptionDepartmet = exempr.getDouble(1);
//
//                                }
                                ...
                                java.sql.PreparedStatement pset12 = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code=code and (department  ilike 'XRY%'  )");
...
//                                exemp = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking'  and activity  ilike 'LAB%' ");
//                                exempr = exemp.executeQuery();
//
//                                while (exempr.next()) {
//                                    exemptionDepartmet = exempr.getDouble(1);
//
//                                }
                                java.sql.PreparedStatement pset121 = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code=code and department  ilike 'LAB'");
...
//                                exemp = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking'  and activity  ilike '%THEATRE%' ");
//                                exempr = exemp.executeQuery();
//
//                                while (exempr.next()) {
//                                    exemptionDepartmet = exempr.getDouble(1);
...
//                                }
                                ...
                                java.sql.PreparedStatement psets1e = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code=code and department  ilike 'THE' ");
                                ...
                                java.sql.PreparedStatement psetd = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code=code and department  ilike 'MOT%'");
...
//                                exemp = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking'  and activity  ilike '%OUT PATIENT%' ");
//                                exempr = exemp.executeQuery();
//
//                                while (exempr.next()) {
//                                    exemptionDepartmet = exempr.getDouble(1);
...
//                                }
                                ...
                                java.sql.PreparedStatement psetp1 = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code=code and ( department  ilike 'OPP' )");
...
//                                exemp = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking'  and activity  ilike '%PHARM%' ");
//                                exempr = exemp.executeQuery();
//
//                                while (exempr.next()) {
//                                    exemptionDepartmet = exempr.getDouble(1);
...
//                                }
                                ...
                                java.sql.PreparedStatement pseth1 = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code=code and department  ilike 'PF' ");
...
//                                exemp = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking'  and (activity  ilike 'Farm%' OR  activity  ilike 'Procurement%' OR activity  ilike 'Eye %' OR activity  ilike 'E.N.T%' OR activity  ilike 'MARTENITY%' OR activity  ilike 'NBU%' OR activity  ilike 'nutri%' OR activity  ilike 'ortho%' OR activity  ilike 'other%' OR  activity  ilike 'VEHICLE%' OR  activity  ilike 'PHYSIO%' OR activity  ilike 'REFERRAL%' OR  activity  ilike 'RECORD%' OR activity  ilike '%CLINICS%' OR activity  ilike 'MCH%'  OR activity  ilike 'DEBTOR%' OR activity  ilike 'PLASTER%' OR activity  ilike 'DENTAL%' OR activity  ilike 'MEDICAL%' OR activity  ilike 'OCCUPATIONAL%' ) ");
//                                exempr = exemp.executeQuery();
//
//                                while (exempr.next()) {
//                                    exemptionDepartmet = exempr.getDouble(1);
...
//                                }
                                ...
                                java.sql.PreparedStatement psetw = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code=code and  department ilike 'AOT' ");//(activity_code NOT LIKE '"+pha+"' OR activity_code NOT LIKE '"+opp+"' OR activity_code NOT LIKE '"+mot+"' OR activity_code NOT LIKE '"+the+"' OR activity_code NOT LIKE '"+xray+"' OR activity_code NOT LIKE '"+mot+"')");
                                java.sql.ResultSet rsetw = psetw.executeQuery();

                                while (rsetw.next()) {
                                    currentAOTTotal = currentAOTTotal + rsetw.getDouble(1) ;
                                    System.err.println("Date for others ..." + rsetw.getDouble(1));
                                }
                                System.err.println("Date for others ..." + currentAOTTotal);
                                System.err.println("Date for others ..." );

                                // }
                                Double tt = currentAOTTotal + currentIPPTotal + currentLABTotal + currentMATTotal + currentMOTTotal + currentOPPTotal + currentPFTotal + currentTHETotal + currentXRYTotal;
                                System.out.println("Add ups/////////////// ............" + tt);
                                String aotDesc = null;
...
////                                Unhif = Unhif + Uexemp;
                                ...
                                // while (rset11agn.next()) {
                                java.sql.PreparedStatement psetwn1 = connectDB.prepareStatement("select code from pb_activity where activity ilike 'IEDS' ");
                                java.sql.ResultSet rsetwn1 = psetwn1.executeQuery();
                                while (rsetwn1.next()) {
                                    java.sql.PreparedStatement psetwn = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection WHERE date = '" + listofAct[i] + "' AND activity_code ILIKE '" + rsetwn1.getString(1) + "'");
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                System.out.println("Janu total....." + janu);
                                System.err.println("Total debtors "+ debtors);
                                ...
                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(dbObject.getDBObject(java.lang.String.valueOf(uhcdebts), "0.00")), pFontHeader1);
                                table.addCell(phrase);

                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(dbObject.getDBObject(java.lang.String.valueOf(freeelderlydebts), "0.00")), pFontHeader1);
                                table.addCell(phrase);
                                ...
                                // java.lang.Object[] listofAct1 = this.getListofActivities1();
                                //  try{
                                java.sql.PreparedStatement psetwb = connectDB.prepareStatement("select activity,sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking' and activity_code=code and department ilike 'AOT' group by activity");//(activity_code NOT LIKE '"+pha+"' OR activity_code NOT LIKE '"+opp+"' OR activity_code NOT LIKE '"+mot+"' OR activity_code NOT LIKE '"+the+"' OR activity_code NOT LIKE '"+xray+"' OR activity_code NOT LIKE '"+mot+"')");
                                java.sql.ResultSet rsetwb = psetwb.executeQuery();
                                String actvity = "";
                                String activityamt = "";
                                //Double exemptionDept = 0.00;
                                Double amtt = 0.00;
                                System.err.println("date.... " + listofAct[i]);
                                while (rsetwb.next()) {
                                    if (rsetwb.getDouble(2) != 0 || rsetwb.getString(2) != null || rsetwb.getString(2) != "") {
//                                        exemp = connectDB.prepareStatement("select sum(debit-credit) FROM ac_cash_collection ac,pb_activity WHERE ac.date = '" + listofAct[i] + "' AND transaction_type != 'Banking'  and (activity  ilike '" + rsetwb.getString(1) + "'  ) ");
//                                        exempr = exemp.executeQuery();
//
//                                        while (exempr.next()) {
//                                            exemptionDept = exempr.getDouble(1);
//
//                                        }
                                        ...
                                        if (len > 12) {
                                            actvity = actvity + rsetwb.getString(1).toLowerCase().substring(0, 12) + "\n";
                                        } else {
                                            actvity = actvity + rsetwb.getString(1).toLowerCase().substring(0, len) + "\n";
                                        }
                                        activityamt = activityamt + amtt + "\n";
                                    }
                                }
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);

                                //phrase = new Phrase(actvity, pFontHeader1); //
                                phrase = new Phrase(actvity, pFontHeader1);
                                table.addCell(phrase);

                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);

//                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(dbObject.getDBObject(java.lang.String.valueOf(activityamt), "0.00")), pFontHeader1);
                                phrase = new Phrase(activityamt, pFontHeader1);
                                table.addCell(phrase);
                                table.getDefaultCell().setColspan(1);
                            ...
                            phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(dbObject.getDBObject(java.lang.String.valueOf(uhctotaldebts), "0.00")), pFontHeader1);
                            table.addCell(phrase);

                            phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(dbObject.getDBObject(java.lang.String.valueOf(freeelderlytotaldebts), "0.00")), pFontHeader1);
                            table.addCell(phrase);
