...
public class DoctorAllocationPdf implements java.lang.Runnable {
    ...
    public void DoctorAllocationPdf(java.sql.Connection connDb, java.lang.String combox,java.util.Date begindate, java.util.Date endate) {
        MNo = combox;
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("Doctor's Allocation - Page: "), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
                    ...
                    try {
                        ...
                        int headerwidths[] = {12,24,11,20,10,12,10,10,15};
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = st.executeQuery("select staff_no,initcap(first_name||' '||middle_name),last_name,estate,tel_no from pb_doctors_list where staff_no = '"+MNo+"'");
                            java.sql.ResultSet rset1 = st1.executeQuery("select input_date::date,INITCAP(patient_no),bill_date,INITCAP(inv_no),cheque_no,SUM(amount_alloc) from dr_allocation where acc_no = '"+MNo+"'and input_date BETWEEN '"+beginDate+"' AND '"+endDate+"'  GROUP BY inv_no,input_date::date,patient_no,bill_date,cheque_no ORDER BY input_date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            
                            
                            table.getDefaultCell().setColspan(9);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                            phrase = new Phrase("Doctor's Allocation)", pFontHeader1);
                            table.addCell(phrase);
                            
                            table.getDefaultCell().setColspan(9);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("A/C No.  "+memNo, pFontHeader1);
                            table.addCell(phrase);
                            ...
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Bill date",pFontHeader1);
                            table.addCell(phrase);
