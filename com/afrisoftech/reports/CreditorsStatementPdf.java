...
public class CreditorsStatementPdf implements java.lang.Runnable {
    ...
    boolean statementDateType = false;
    ...
    public void CreditorsStatementPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate, java.lang.String combox, boolean dateType) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {7, 15, 15, 30, 15, 15, 15, 15};
                    ...
                    try {
                        ...
                        int headerwidths[] = {7, 15, 25, 12, 12, 10, 12, 15, 15, 15};
                        ...
                        try {
                            ...
                            java.sql.ResultSet rsetTotals = null;
                            if (statementDateType) {
                                rset3 = st3.executeQuery("select hospital_name,postal_code||' '||box_no||' '||town,main_telno||' '||other_telno,initcap(street),main_faxno,email,website,room_no from pb_hospitalprofile");
                                rset4 = st4.executeQuery("select distinct supplier_name,code from st_suppliers where supplier_name ilike '" + memNo + "' GROUP BY supplier_name,code");
                                rset1 = st1.executeQuery("select date,reason,voucher_no, transaction_type,invoice_no,sum(debit),sum(credit),journal_no from ac_accounts_payable where dealer ilike '" + memNo + "' AND date::date BETWEEN '" + beginDate + "' AND '" + endDate + "' GROUP BY date,reason,voucher_no, transaction_type,invoice_no,journal_no order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                                rsetT = st41.executeQuery("select sum(credit-debit) from ac_accounts_payable where dealer ilike '" + memNo + "' AND date::date < '" + beginDate + "'");
                                rsetTotals = st2.executeQuery("select sum(debit),sum(credit) from ac_accounts_payable where dealer ilike '" + memNo + "' AND date::date BETWEEN '" + beginDate + "' AND '" + endDate + "'");
                            } else {
                                rset3 = st3.executeQuery("select hospital_name,postal_code||' '||box_no||' '||town,main_telno||' '||other_telno,initcap(street),main_faxno,email,website,room_no from pb_hospitalprofile");
                                rset4 = st4.executeQuery("select distinct supplier_name,code from st_suppliers where supplier_name ilike '" + memNo + "' GROUP BY supplier_name,code");
                                rset1 = st1.executeQuery("select date,reason,voucher_no, transaction_type,invoice_no,sum(debit),sum(credit),journal_no from ac_accounts_payable where dealer ilike '" + memNo + "' AND invoice_date::date BETWEEN '" + beginDate + "' AND '" + endDate + "' GROUP BY date,reason,voucher_no, transaction_type,invoice_no,journal_no order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                                rsetT = st41.executeQuery("select sum(credit-debit) from ac_accounts_payable where dealer ilike '" + memNo + "' AND invoice_date::date < '" + beginDate + "'");
                                rsetTotals = st2.executeQuery("select sum(debit),sum(credit) from ac_accounts_payable where dealer ilike '" + memNo + "' AND invoice_date::date BETWEEN '" + beginDate + "' AND '" + endDate + "'");
