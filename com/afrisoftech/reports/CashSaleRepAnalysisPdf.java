...
public class CashSaleRepAnalysisPdf implements java.lang.Runnable {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("Receipts Summary Report - Page: ", pFontHeader), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
...
//                            java.lang.Object[] listofAct = this.getListofActivities();
                            ...
                            java.sql.ResultSet rset = st.executeQuery("SELECT activity, activity_code, SUM(amt) FROM (SELECT  pb.activity,ac.activity_code, SUM(ac.debit-ac.credit) as amt from \n"
                                    + "ac_cash_collection ac ,pb_activity pb where ac.date BETWEEN '"+beginDate+"' AND '"+endDate+"'  \n"
                                    ...
                                    + "UNION ALL\n"
                                    + "SELECT ac.main_service_,pb.code, SUM(ac.amount_allocated_) AS amt from funsoft_revenue_break_down('"+beginDate+"' , '"+endDate+"') \n"
                                    + "ac ,pb_activity pb where    UPPER(pb.activity) = UPPER(ac.main_service_)   GROUP BY 1, 2 ) AS foo GROUP BY 1,2 ORDER BY 3 DESC");
                            ...
                            while (rset.next()) {
                                table.getDefaultCell().setColspan(1);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(rset.getObject(2).toString().toUpperCase(),pFontNum);
