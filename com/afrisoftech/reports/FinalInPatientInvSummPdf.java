...
public class FinalInPatientInvSummPdf implements java.lang.Runnable {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths1[] = {15, 15, 30, 15, 15, 15};
                        ...
                        try {
                            ...
                            java.sql.Statement std22 = connectDB.createStatement();
                            ...
                            java.sql.ResultSet rseta = sta.executeQuery("SELECT DISTINCT initcap(ad.ward),initcap(ad.bed_no),initcap(ad.doctor), sub_chief FROM hp_admission ad,hp_patient_card pr WHERE pr.invoice_no = '" + memNo + "' and pr.patient_no = ad.patient_no "
                                    + "UNION ALL SELECT DISTINCT initcap(ad.deseased_sourse),initcap(''),initcap(''), initcap('') FROM hp_mortuary ad WHERE ad.invoice_no = '" + memNo + "' ");
                            ...
                            String schemeName = "";
                            String memberNumber = "";
                            java.sql.PreparedStatement pstmtPayer = connectDB.prepareStatement("SELECT payee, dealer, journal_no from ac_debtors WHERE invoice_no = ?");
                            pstmtPayer.setString(1, memNo);
                            java.sql.ResultSet rsetPayer = pstmtPayer.executeQuery();
                            while(rsetPayer.next()){
                                payerName = rsetPayer.getString(1);
                                schemeName = rsetPayer.getString(2);
                                memberNumber = rsetPayer.getString(3);
                            }
                            ...
                            while (rset.next()) {
                                ...
                                phrase = new Phrase("Payer : " +payerName, pFontHeader1);

                                table1.addCell(phrase);

                                table1.getDefaultCell().setColspan(3);
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase1 = new Phrase("Scheme Name : "+schemeName, pFontHeader1);
                                //table1.addCell(phrase);
                            ...
                            while (rseta.next()) {
                                ...
                                //while (rseta.next())
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase6 = new Phrase("Unit No: " + dbObject.getDBObject(rseta.getObject(4), "-"), pFontHeader1);
                                // table1.addCell(phrase);
                            ...
                            while (rsetb.next()) {
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Member No.: " + memberNumber, pFontHeader1);
                                table1.addCell(phrase);
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery(" select initcap(main_service) as service,sum(dosage)::int,sum(debit-credit) from hp_patient_card where invoice_no = '" + memNo + "' and paid = 'true' and service != 'N.H.I.F' AND service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj' AND service not ilike 'Discou%' AND service != 'Invoice' AND service not ilike 'nhif diff%' group by initcap(main_service) order by service");
                            java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(debit - credit) from hp_patient_card where invoice_no = '" + memNo + "' and service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj' AND service not ilike 'Discou%' AND service != 'Invoice' AND service not ilike 'nhif diff%'");

                            java.sql.ResultSet rset11 = st11.executeQuery(" select CASE WHEN (service ilike 'invoice%') THEN initcap(scheme) ELSE initcap(main_service) END AS service,CASE WHEN (service = 'Receipt') THEN requisition_no ELSE reference END AS reference,sum(credit-debit) from hp_patient_card where invoice_no = '" + memNo + "' AND (service = 'N.H.I.F' or service ilike 'Receipt%' or service ilike 'Receipt Adj'  OR service ilike 'Discou%'  OR service ilike 'nhif diff%'   OR service ilike 'invoice%') group by main_service,reference,service,scheme,requisition_no order by 1");
                            java.sql.ResultSet rsetTotals1 = st21.executeQuery("select sum(credit-debit) from hp_patient_card where invoice_no = '" + memNo + "' and (service = 'N.H.I.F' or service ilike 'Receipt' or service ilike 'Receipt Adj'  OR service ilike 'Discou%'  OR service ilike 'nhif diff%')");
