...
public class TransactionsListSumPdf implements java.lang.Runnable {
    ...
    public void TransactionsListSumPdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("Off Balance Transactions List :     Period : From "  +dateFormat.format(endDate11)+" To "+dateFormat.format(endDate1), pFontHeader);
                        ...
                        phrase = new Phrase("Audit No",pFontHeader);
                        table.addCell(phrase);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = st.executeQuery("select date,CASE WHEN (transaction_no IS NULL) THEN '-' ELSE transaction_no END AS transaction_no,sum(debit) as debit,sum(credit) as credit,sum(balance) as balance from trans_list_summary WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND balance <> 0 GROUP BY date,transaction_no ORDER BY date,transaction_no ASC");
                            
                            java.sql.ResultSet rsetTotals = st2.executeQuery("SELECT SUM(debit) as debit,SUM(credit) as credit,sum(balance) as balance from trans_list_summary WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND balance <> 0");
