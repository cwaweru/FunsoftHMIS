...
public class CashBookListPdf implements java.lang.Runnable {
    ...
    com.lowagie.text.Font pFontHeader = FontFactory.getFont(FontFactory.TIMES, 7, Font.BOLD);
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {5, 12, 15, 20, 23, 10, 10, 10, 13, 13, 13};
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = st.executeQuery("select date,activity_code,initcap(dealer),initcap(description),voucher_no, slip_no, cheque_no,debit,credit from ac_cash_book WHERE date BETWEEN '" + beginDate + "' AND '" + endDate + "' AND account_no = '" + bank + "' ORDER BY date,cheque_no");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
                            ...
                            while (rset.next()) {
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(dbObject.getDBObject(rset.getObject(3),"-").toString(), pFontHeader1);

                                table.addCell(phrase);

                                phrase = new Phrase(dbObject.getDBObject(rset.getObject(4),"-").toString(), pFontHeader1);
//dbObject.getDBObject(rsetGetCode.getObject(5).toString(),"-")
                                table.addCell(phrase);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(dbObject.getDBObject(rset.getObject(5),"-").toString(), pFontHeader1);
                                System.out.println(dbObject.getDBObject(rset.getObject(5),"-").toString());
                                table.addCell(phrase);

                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(dbObject.getDBObject(rset.getObject(6),"-").toString(), pFontHeader1);

                                table.addCell(phrase);

                                phrase = new Phrase(dbObject.getDBObject(rset.getObject(7),"-").toString(), pFontHeader1);
                                ...
                                osBalancebf = osBalancebf + rset.getDouble(9);
