...
public class MOH301InPatientPdf implements java.lang.Runnable {
    ...
    public void MOH301InPatientPdf(java.sql.Connection connDb, java.util.Date startDate, java.util.Date lastDate) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("MOH - 301_IN-Patient Register - Page: "), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("IN-PATIENT REGISTER - MOH 301_IN_Patient", pFontHeader2);
                            ...
                            phrase = new Phrase("IN_PATIENT REGISTER", pFontHeader22);
                            table.addCell(phrase);
                            ...
                            phrase = new Phrase("Date of Admission", pFontHeader11);
                            table.addCell(phrase);

                            phrase = new Phrase("IN-Patient No.", pFontHeader11);
                            table.addCell(phrase);
                            ...
                            phrase = new Phrase("HIV Status (1 = Known Positive, 2 = Positive this visit)", pFontHeader11);
                            table.addCell(phrase);

                            phrase = new Phrase("HIV+ (1 = Nutrition Education, 2 = Nutrition Suppliments)", pFontHeader11);
                            table.addCell(phrase);

                            phrase = new Phrase("Diagnosis", pFontHeader11);
                            table.addCell(phrase);

                            phrase = new Phrase("Treatment and Prescription No.", pFontHeader11);
                            table.addCell(phrase);

                            phrase = new Phrase("Date of Discharge", pFontHeader11);
                            table.addCell(phrase);

                            phrase = new Phrase("Outcome (A = Alive, D = Dead)", pFontHeader11);
                            table.addCell(phrase);
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                java.sql.PreparedStatement stw = connectDB.prepareStatement("SELECT"
                                        ...
                                        + "DESC LIMIT 1) as village, tel, date_part('day', discharge_date::date)"
                                        + " ||'-'||date_part('month', discharge_date::date) ||'-'||date_part('year', discharge_date::date), "
                                        + "(SELECT (CASE WHEN admission_outcome ILIKE 'died' THEN '2' ELSE '1' END)"
                                        + "  FROM hp_patient_diagnosis WHERE hp_patient_diagnosis.patient_no"
                                        + " = hp_admission.patient_no ORDER BY 1 LIMIT 1)  as outcome, reffered_from  FROM hp_admission WHERE"
                                        + " discharge_date::date BETWEEN '" + beginDate + "' AND '" + endDate + "' "
                                        + "AND patient_no = ?");
    ...
    public java.lang.Object[] getListofStaffNos() {
        ...
        try {
            ...
            System.out.println("Started reading patient names ...[" + beginDate.toString() + "] [" + endDate.toString() + "]");


            java.sql.PreparedStatement stmt1 = connectDB.prepareStatement("SELECT DISTINCT patient_no, patient_name FROM hp_admission where discharge_date::date BETWEEN '" + beginDate + "' AND '" + endDate + "' ORDER BY patient_name ASC");
