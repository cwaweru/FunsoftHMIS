...
public class SchemeStaffKarenPdf implements java.lang.Runnable {
    ...
    com.lowagie.text.Font pFontHeader1b = FontFactory.getFont(FontFactory.HELVETICA, 8, Font.ITALIC);
    ...
    public void SchemeStaffKarenPdf(java.sql.Connection connDb, java.lang.String combox1,java.lang.String combox) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("Schemes Staff List (arranged alphabetically)                                                                                            Page: ", pFontHeader1b), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
                    ...
                    try {
                        ...
                        int headerwidths[] = {8, 30,25,25,12};
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select staff_no,staff_name,department, CASE WHEN suspended ilike 'false' THEN 'ACTIVE' ELSE 'SUSPENDED' END AS suspended from hp_schemestaff where account_no = '"+memNo+"' order by date_set");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                            java.sql.ResultSet rset11 = st11.executeQuery("select count(staff_no) from hp_schemestaff where account_no = '"+memNo+"'");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"'and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                            ...
                            phrase = new Phrase("Printed On " +datenowSql, pFontHeader1);
                            table.addCell(phrase);
                           ...
                           */
                                while (rset41.next()){
                                    ...
                                    table.getDefaultCell().setColspan(5);
                                    phrase = new Phrase("Scheme Payer: "+rset41.getObject(1).toString(), pFontHeader1);
                                    table.addCell(phrase);
                          ...
                          */
                                    ...
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase("Scheme Account No : " +rset4.getObject(2).toString(), pFontHeader1);
                                    table.addCell(phrase);
                                    // table.getDefaultCell().setColspan(2);
                                }
                            }
                            ...
                            while (rset1.next()){
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(dbObject.getDBObject(seq + ".", "-"), pFontHeader);
                                table.addCell(phrase);
