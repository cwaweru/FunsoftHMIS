                    ...
                    try {
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                int cash1 = 0;
                                int scheme1 = 0;
                                java.sql.PreparedStatement pset = connectDB.prepareStatement("select count(distinct(patient_no)) from hp_patient_visit  WHERE date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and transaction_type ilike 'regi%' and payment not ilike 'scheme' and date = ?::date");
                                pset.setString(1,listofAct[i].toString());
                                java.sql.ResultSet rset = pset.executeQuery();
                                java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select count(distinct(patient_no)) from hp_patient_visit  WHERE date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and transaction_type ilike 'regi%' and payment ilike 'scheme' and date = ?::date");
