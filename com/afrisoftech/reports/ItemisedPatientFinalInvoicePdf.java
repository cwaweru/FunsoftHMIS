...
public class ItemisedPatientFinalInvoicePdf implements java.lang.Runnable {
    ...
    public void ItemisedPatientFinalInvoicePdf(java.sql.Connection connDb, java.lang.String combox, java.lang.String invoiceNO, java.lang.String payer) {
        MNo = combox;
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("Itemised Final Invoice".toUpperCase(), pFontHeaderX);
                            table1.addCell(phrase);
                            table1.getDefaultCell().setFixedHeight(16);
                            table1.getDefaultCell().setBorder(Rectangle.BOX);
                            table1.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery(" select initcap(main_service) as service,sum(dosage)::int,sum(debit-credit) from hp_patient_card where visit_id = '" + memNo + "' and service != 'N.H.I.F' AND service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj'  AND service not ilike '%DIFF ' AND service not ilike 'Discou%' AND service != 'Invoice' group by initcap(main_service) having sum(debit-credit)>0 order by service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(debit - credit) from hp_patient_card where visit_id = '" + memNo + "' and service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj'   AND service not ilike '%DIFF '  AND service not ilike 'Discou%' AND service != 'Invoice' having sum(debit-credit)>0");

                            java.sql.ResultSet rset11 = st11.executeQuery(" select initcap(service) as service,sum(dosage)::int,sum(credit-debit) from hp_patient_card where visit_id = '" + memNo + "' "
                                    + "---AND (service = 'N.H.I.F' or service ilike 'Receipt%' or service ilike 'Receipt Adj'  or service ilike '%DIFF '  OR service ilike 'Discou%')\n"
                                    + " group by  service having sum(credit-debit)>0 order by service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                         ...
                         //   phrase = new Phrase("Balance ".toUpperCase() + ks.toUpperCase(), pFontHeader1);
                            ...
                            while (rset1.next()) {
                                ...
                                java.sql.ResultSet rset1a = st1a.executeQuery("  select initcap(service) as service,sum(dosage::int),sum(debit-credit) from hp_patient_card where visit_id = '" + MNo + "' and service != 'N.H.I.F' AND service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj'  AND service not ilike '%DIFF ' AND service not ilike 'Discou%' AND service != 'Invoice' and main_service ilike '" + rset1.getString(1) + "' group by service  having sum(debit-credit)>0 order by service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                                ...
                                while (rset1a.next()) {
                                    ...
                                    phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rset1a.getString(3)), pFontHeader);

                                    table.addCell(phrase);
                                    table.getDefaultCell().setColspan(1);
                                    osBalance = osBalance + rset1a.getDouble(3);
                                    subtotal = subtotal + rset1a.getDouble(3);
