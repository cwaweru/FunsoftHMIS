                ...
                try {
                    ...
                    try {
                       ...
                       // table.addCell(phrase);
                        ...
                        try {
                            java.lang.Object[] listofAct = this.getListofActivitiesb();
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                java.sql.ResultSet rsetTotals = st2.executeQuery("SELECT code, activity from pb_activity WHERE code = '"+listofAct[i]+"'");
                                
                                java.sql.ResultSet rset = st.executeQuery("select date,CASE WHEN (activity_code IS NULL) THEN '-' ELSE activity_code END AS activity_code ,CASE WHEN (transaction_no IS NULL) THEN '-' ELSE transaction_no END AS transaction_no,initcap(description),initcap(transaction_type),sum(debit),sum(credit) from transaction_list_view WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND activity_code = '"+listofAct[i]+"' GROUP BY date,activity_code,description,transaction_no,transaction_type ORDER BY transaction_no ASC");
...
docPdf.close();  com.afrisoftech.lib.PDFRenderer.renderPDF(tempFile);
    ...
    public java.lang.Object[] getListofActivitiesb() {
