...
public class DivisionalShiftSurrenderPdf implements java.lang.Runnable {
    ...
    public void DivisionalShiftSurrenderPdf(java.sql.Connection connDb, java.lang.String combox, java.lang.String userName) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("CASHIER'S SHIFT REPORT SUMMARY by DIVISION", pFontHeaderB);
                            table.addCell(phrase);
...
//                                java.lang.Object[] listofAct1 = this.getListofActivities1(listofAct[i]);
                            ...
                            // table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Shift Breakdown Summary by Division".toUpperCase(), pFontHeaderB);
                            ...
                            java.sql.ResultSet rsets = sts.executeQuery("select (SELECT DISTINCT branch from pb_activity where pb_activity.code = ac_cash_collection.activity_code),(SELECT DISTINCT donor FROM ac_branch where code in (SELECT branch from pb_activity where pb_activity.code = ac_cash_collection.activity_code) ) ,SUM(debit-credit) from ac_cash_collection where shift_no = '" + memNo + "'  GROUP BY 1, 2 ORDER BY 1");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
...
////                                java.sql.ResultSet rsets = sts.executeQuery("select DISTINCT upper(activity_code),SUM(debit-credit),price_category from ac_cash_collection where shift_no = '" + memNo + "'  AND activity_code = '" + listofAct11[x].toString() + "' GROUP BY activity_code,price_category ORDER BY price_category ");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
                            //   java.sql.ResultSet rsetsw = stsw.executeQuery("select (SELECT depart_name FROM pb_main_department WHERE pb_main_department.department_code = pb_activity.country_name) from pb_activity where code = '" + listofAct11[x].toString() + "' order by 1 ");
                            //table.getDefaultCell().setBackgroundColor(java.awt.Color.WHITE);
                            table.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            table.getDefaultCell().setColspan(2);
                            phrase = new Phrase("DIVISION CODE", pFontHeaderB);

                            table.addCell(phrase);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            table.getDefaultCell().setColspan(4);
                            phrase = new Phrase("DIVISION NAME", pFontHeaderB);

                            table.addCell(phrase);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                            table.getDefaultCell().setColspan(2);
                            phrase = new Phrase("AMOUNT ("+ks+")", pFontHeaderB);

                            table.addCell(phrase);
                            while (rsets.next()) {
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                table.getDefaultCell().setColspan(2);
                                phrase = new Phrase(rsets.getString(1), pFontHeader1);
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(new com.afrisoftech.lib.DBObject().getDBObject(rsets.getObject(2), "-").toUpperCase(), pFontHeader1);

                                table.addCell(phrase);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                table.getDefaultCell().setColspan(2);
                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rsets.getString(3)),
                                        pFontHeader1);
    ...
    public java.lang.Object[] getListofActivities11() {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT (SELECT country_name FROM pb_activity WHERE pb_activity.code = ac_cash_collection.activity_code) FROM ac_cash_collection where shift_no = '" + memNo + "' order by 1");
