...
public class TrialBalanceBFPdf implements java.lang.Runnable {
    ...
    public void TrialBalanceBFPdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("Trial Balance Monthly Movement:     Period : From "  +dateFormat.format(endDate11)+" To "+dateFormat.format(endDate1), pFontHeader);
                        ...
                        try {
                           ...
                           /* java.sql.Statement st2x = connectDB.createStatement();
                            ...
                            java.sql.ResultSet rsetx = st2c.executeQuery("SELECT SUM(debit - credit) from tb_itemized_summary WHERE date between '"+endDate+"'");
                            while (rsetx.next()) {
                            ...
                            */
                            ...
                            java.sql.ResultSet rset = st.executeQuery("SELECT tb.gl_code,sum(debit-tb.credit) from tb_itemized_summary tb,pb_activity pb WHERE tb.date <= '"+endDate+"' AND pb.category_class not ilike 'P%' AND tb.gl_code = pb.code  group by 1 UNION ALL SELECT tb.gl_code,sum(debit-tb.credit) from tb_itemized_summary tb, pb_activity pb WHERE tb.date BETWEEN  '"+beginDate+"' AND '"+endDate+"' AND pb.category_class ilike 'P%' AND tb.gl_code = pb.code group by 1 order by 1");
                            
                            java.sql.ResultSet rsetw = st22.executeQuery("SELECT sum(debit-tb.credit) from tb_itemized_summary tb,pb_activity pb WHERE tb.date < '"+beginDate+"' AND pb.category_class ilike 'P%' AND tb.gl_code = pb.code");
                            ...
                            while (rsetw.next()) {
                                debit1 = rsetw.getDouble(1);
                                java.sql.ResultSet rset13 = stx.executeQuery("SELECT DISTINCT code,CASE WHEN (activity IS NOT NULL) THEN initcap(activity) ELSE '-' END AS activity FROM pb_activity WHERE activity_category ilike 'pl'");
                                while (rset13.next()) {
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase(dbObject.getDBObject(rset13.getObject(1), "-"), pFontHeader1);
                                    
                                    // phrase = new Phrase(rset1.getString(1), pFontHeader1);
                                    table.addCell(phrase);
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase(dbObject.getDBObject(rset13.getObject(2), "-"), pFontHeader1);
                                    table.addCell(phrase);
                                }
                                if(debit1 > 0){
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
