...
public class StatementPatientPdf implements java.lang.Runnable {
    ...
    public void StatementPatientPdf(java.sql.Connection connDb, java.lang.String combox) {
        MNo = combox;
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("Patient Statement".toUpperCase(), pFontHeaderX);
                            table1.addCell(phrase);
                            table1.getDefaultCell().setFixedHeight(16);
                            table1.getDefaultCell().setBorder(Rectangle.BOX);
                            table1.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery(" select initcap(main_service) as service,sum(dosage)::int,sum(debit-credit) from hp_patient_card where visit_id = '" + memNo + "' and service != 'N.H.I.F'  AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj'  AND service not ilike '%DIFF ' AND service not ilike 'Discou%' AND service != 'Invoice' and credit>=0 group by initcap(main_service) order by service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            ...
                            java.sql.ResultSet rset11 = st11.executeQuery(" select initcap(main_service) as service,sum(dosage)::int,sum(credit-debit) from hp_patient_card where visit_id = '" + memNo + "' AND (service = 'N.H.I.F' or service ilike 'Receipt%' or service ilike 'Receipt Adj'  or service ilike '%DIFF '  OR service ilike 'Discou%') group by main_service order by main_service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            java.sql.ResultSet rsetTotals1 = st21.executeQuery("select sum(credit) from hp_patient_card where visit_id = '" + memNo + "' and (service = 'N.H.I.F' or service ilike 'Receipt' or service ilike 'Receipt Adj'  OR service ilike 'Discou%'  or service ilike '%DIFF ')");
