...
import static com.afrisoftech.reports.FinalInterimPatientlnvSummPdf.connectDB;
...
public class FinalInPatientlnvSummPdf2 implements java.lang.Runnable {
                ...
                try {
                 ...
                 java.sql.Statement st321 = connectDB.createStatement();
                           ...
                           //  String visitId = "";
                            ...
                            java.sql.ResultSet rsetb = stb.executeQuery("select distinct (hr.discharge_date - hr.adm_date),hr.account_no, hr.discharge_date from hp_inpatient_register hr,hp_patient_card pc where pc.patient_no = hr.patient_no and pc.visit_id = '"+visitId+"'");
                            
                            java.sql.ResultSet rseta = sta.executeQuery("select distinct initcap(ad.ward),initcap(ad.bed_no),initcap(ad.doctor) from hp_admission ad,hp_patient_card pr where pr.visit_id = '"+visitId+"' and pr.patient_no = ad.patient_no");
                            
                            java.sql.ResultSet rset = st.executeQuery("select distinct pr.patient_no,initcap(pr.first_name||' '||pr.second_name||' '||pr.last_name),pr.address,pr.residence,pr.tel_no,pr.payer,pr.description,pr.category from hp_inpatient_register pr,hp_patient_card ac where ac.visit_id = '"+visitId+"' and ac.patient_no = pr.patient_no");
                            ...
                            table1.getDefaultCell().setColspan(6);
                            table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                            phrase = new Phrase("Final Invoice", pFontHeader11);
                            table1.addCell(phrase);
                            table1.getDefaultCell().setBorder(Rectangle.BOTTOM);
                            table1.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery(" select initcap(main_service) as service,sum(dosage)::int,sum(debit-credit) from hp_patient_card where visit_id = '"+visitId+"' and paid = 'true' and service != 'N.H.I.F' AND service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj' AND service not ilike 'Discou%' AND service != 'Invoice' group by initcap(main_service) order by service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(debit - credit) from hp_patient_card where visit_id = '"+visitId+"' and service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj' AND service not ilike 'Discou%' AND service != 'Invoice'");
                            
                            java.sql.ResultSet rset11 = st11.executeQuery(" select initcap(main_service) as service,sum(dosage)::int,sum(credit-debit) from hp_patient_card where visit_id = '"+visitId+"' AND (service = 'N.H.I.F' or service ilike 'Receipt%' or service ilike 'Receipt Adj'  OR service ilike 'Discou%') group by main_service order by main_service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            java.sql.ResultSet rsetTotals1 = st21.executeQuery("select sum(credit) from hp_patient_card where visit_id = '"+visitId+"' and (service = 'N.H.I.F' or service ilike 'Receipt' or service ilike 'Receipt Adj'  OR service ilike 'Discou%')");
