...
public class DetailedRevenueDetailedReportPdf implements java.lang.Runnable {
    ...
    public void DetailedRevenueDetailedReportPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                while (rset1.next()) {
                                    ...
                                    java.sql.ResultSet rset = st5.executeQuery("select TRIM(initcap(service_type)), (select sum(qty) from (select sum(quantity) as qty from ac_cash_collection where upper(ac_cash_collection.description) = upper(ac_ledger.service_type) and ac_cash_collection.date BETWEEN '" + beginDate + "' AND '" + endDate + "' AND ac_cash_collection.activity_code = '" + listofAct[i].toString() + "'  UNION select sum(dosage) as qty from hp_patient_card where upper(hp_patient_card.service) = upper(ac_ledger.service_type) and hp_patient_card.date BETWEEN '" + beginDate + "' AND '" + endDate + "' AND upper(hp.main_service) = upper('"+com.afrisoftech.lib.GLCodesFactory.getActivityDescription(connectDB, listofAct[i].toString())+"') as foo), sum(credit-debit) from ac_ledger where date BETWEEN '" + beginDate + "' AND '" + endDate + "' AND activity_code = '" + listofAct[i].toString() + "' group by service_type, ac_ledger.activity_code, ac_ledger.description, ac_ledger.date order by service_type");
                                    java.sql.Statement st52 = connectDB.createStatement();

                                   // sum(credit-debit)/(select sum(qty) from (select sum(quantity) as qty from ac_cash_collection where upper(ac_cash_collection.description) = upper(ac_ledger.service_type) AND date BETWEEN '" + beginDate + "' AND '" + endDate + "' UNION select sum(dosage) as qty from hp_patient_card where upper(hp_patient_card.service) = upper(ac_ledger.service_type) AND date BETWEEN '" + beginDate + "' AND '" + endDate + "' ) as foo) as rate
                                    
                                    java.sql.ResultSet rset2 = st52.executeQuery("select count(service_type) from ac_ledger where date BETWEEN '" + beginDate + "' AND '" + endDate + "' AND activity_code  = '" + listofAct[i].toString() + "'  group by service_type order by service_type");
                                    while (rset2.next()) {
                                        descr = rset2.getInt(1);
                                    }

                                    while (rset.next()) {
                                        if(rset.getObject(1).toString().contains("Bed")){
                                            System.out.println(com.afrisoftech.lib.GLCodesFactory.getActivityDescription(connectDB, listofAct[i].toString()) +" count : "+ rset.getObject(1).toString());
                                        }
                                        String itemCode = null;
                                        double itemPrice = 0.00;
                                        java.sql.PreparedStatement pstmtItemCode = connectDB.prepareStatement("SELECT code, rate FROM pb_operating_parameters WHERE upper(service_type) ilike upper(?) AND gl_account = ? UNION ALL select product_id as code,selling_price as rate FROM st_stock_prices where upper(product) ilike upper(?) and gl_code = ?  ORDER BY 1 LIMIT 1");
...
//                                        } 
                                        ...
                                        if (numb == 1) {
                                            ...
                                            table.getDefaultCell().setColspan(1);
                                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                            phrase = new Phrase(new com.afrisoftech.lib.DBObject().getDBObject(rset.getObject(2), ""), pFontHeader1);
                                            table.addCell(phrase);
