...
public class FinalDescInPatientIntmDeptlnvPdf implements java.lang.Runnable {

    java.lang.String MNo = null;
    java.lang.String mainService = null;
    ...
    public void FinalDescInPatientIntmDeptlnvPdf(java.sql.Connection connDb, java.lang.String combox, java.lang.String serviceType) {
        MNo = combox;
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rsetb = stb.executeQuery("select distinct (hr.discharge_date - hr.adm_date),hr.account_no, hr.discharge_date from hp_inpatient_register hr,hp_admission pc where pc.patient_no = hr.patient_no and pc.visit_id = '" + memNo + "'");
                            ...
                            java.sql.ResultSet rset = st.executeQuery("select distinct pr.patient_no,initcap(pr.first_name||' '||pr.second_name||' '||pr.last_name),pr.address,pr.residence,pr.tel_no,pr.payer,pr.description,pr.category from hp_inpatient_register pr,hp_admission ac where ac.visit_id = '" + memNo + "' and ac.patient_no = pr.patient_no GROUP BY pr.patient_no,initcap(pr.first_name||' '||pr.second_name||' '||pr.last_name),pr.address,pr.residence,pr.tel_no,pr.payer,pr.description,pr.category");
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rsetTotals1 = st21.executeQuery("select sum(credit-debit) from hp_patient_card where visit_id = '" + memNo + "' and (service = 'N.H.I.F' or service ilike 'Receipt' or service ilike 'Receipt Adj'  OR service ilike 'Discou%')");
                            ...
                            java.sql.ResultSet rset11 = st11.executeQuery(" select date::date,initcap(service) as service,dosage,CASE WHEN (service = 'Receipt') THEN requisition_no ELSE reference END AS reference,credit-debit from hp_patient_card where visit_id = '" + memNo + "' AND (service = 'N.H.I.F' or service ilike 'Receipt' or service ilike 'Receipt Adj'  OR service ilike 'Discou%') order by date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");

                            java.lang.Object listofStaffNos1[] = this.getListofStaffNos1();

                            for (int k = 0; k < listofStaffNos1.length; k++) {
                                double osBalance1 = 0.00;
                                java.sql.ResultSet rset1 = st1.executeQuery(" select date::date,initcap(service) as service,dosage::int,reference,debit-credit from hp_patient_card where visit_id = '" + memNo + "' and service != 'N.H.I.F' AND service not ilike 'Receipt' AND service not ilike 'Receipt Adj' AND service not ilike 'Discou%' AND service != 'Invoice'  AND main_service ilike '" + listofStaffNos1[k] + "' order by date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
    ...
    public java.lang.Object[] getListofStaffNos1() {
        ...
        try {
            ...
            java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT distinct upper(main_service) as main_service FROM hp_patient_card WHERE visit_id = '" + MNo + "' and debit > 0 AND main_service ilike '" + mainService + "' order by main_service");
