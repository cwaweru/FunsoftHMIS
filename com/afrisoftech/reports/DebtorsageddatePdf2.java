...
public class DebtorsageddatePdf2 implements java.lang.Runnable {
    double Credit = 0.00;
    ...
    public void DebtorsageddatePdf2(java.sql.Connection connDb,java.util.Date begindate,java.util.Date endate, java.lang.String bal, java.lang.String acType) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                java.sql.PreparedStatement pSeta1 = connectDB.prepareStatement(""
                                        + "SELECT count(account_no) FROM ac_debtors where  account_no = '"+listofAct[i]+"' and activity_code = '"+glAc+"' order by 1 ASC ");
                                java.sql.PreparedStatement pset22 = connectDB.prepareStatement(""
                                        + "select distinct account_no||' '||scheme_name from ac_schemes WHERE account_no = ? order by 1 ASC");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                if (name > 0){
                                    ...
                                    while (rset22.next()){
                                        ...
                                        java.sql.PreparedStatement pset111 = connectDB.prepareStatement("select sum(debit-credit) from ac_debtors WHERE account_no = ?  AND date <= '"+endDate+"' and activity_code = '"+glAc+"' ");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                        ...
                                        java.sql.PreparedStatement pset112 = connectDB.prepareStatement("select sum(credit) from ac_debtors WHERE account_no = ? AND date <= '"+endDate+"' and activity_code = '"+glAc+"' ");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                        ...
                                        for (int t = ageingDates.length - 1; t >= 0; t--) {
                                            ...
                                            java.sql.PreparedStatement pset = connectDB.prepareStatement("select sum(debit),invoice_no from ac_debtors "
                                                    + "WHERE account_no = ?  AND date between '"+ageingDates[t][0]+"' AND '"+ageingDates[t][1]+"' and activity_code = '"+glAc+"'   "
                                            ...
                                            while (rset.next()){
                                                ...
                                                java.sql.PreparedStatement psetx = connectDB.prepareStatement("select sum(credit),invoice_no from ac_debtors "
                                                        + "WHERE account_no = ?  AND date <= '"+endDate+"' AND invoice_no = '"+invoiceNo+"'  and activity_code = '"+glAc+"'  "
                                                        + "GROUP BY invoice_no order by 2 ASC");
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
            ...
            if(balnc.equalsIgnoreCase("both")){
               java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT CASE WHEN (account_no IS NULL) THEN '-' ELSE account_no END AS account_no,sum(debit-credit) as balance FROM ac_debtors WHERE date::date <= '"+endDate+"' and activity_code = '"+glAc+"' and account_no is not null group by account_no HAVING sum(debit-credit) <> 0 "
                       + "order by 1 ASC");
                
                //java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT account_no,payee FROM ac_debtors where account_no IS NOT NULL and account_no !='' and bal <> 0 order by account_no");
                ...
                if(balnc.equalsIgnoreCase("neg")){
                    ...
                    java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT CASE WHEN (account_no IS NULL) THEN '-' ELSE account_no END AS account_no,sum(debit-credit) as balance"
                            + " FROM ac_debtors WHERE date::date <= '"+endDate+"'  and activity_code = '"+glAc+"'  group by account_no HAVING sum(debit-credit) < 0 order by 1 ASC");
                    java.sql.ResultSet rSet1 = pSet1.executeQuery();
                    while (rSet1.next()) {
                        System.out.println(rSet1.getObject(1).toString());
                        listActVector.addElement(rSet1.getObject(1).toString());
