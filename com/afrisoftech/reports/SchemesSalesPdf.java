    ...
    public void generatePdf() {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        phrase = new Phrase("Growth "+ks,pFontHeader);
                        table.addCell(phrase);
                    ...
                    double osBalance = 0.00;
                            ...
                            for (int j = 0; j < listofStaffNos.length; j++) {
                                ...
                                java.sql.ResultSet rset = st.executeQuery("select db.account_no,upper(db.scheme_name) from ac_schemes db WHERE db.account_no = '"+listofStaffNos[j]+"' ORDER BY 2 ASC");// and (transaction_type = 'Raise Invoice' OR transaction_type = 'Stock Delivery' OR transaction_type = 'Stock Returns') and (description = 'Creditors Payable' OR description = 'Other Payables') GROUP BY invoice_no,date,dealer,item ORDER BY date");
                                java.sql.ResultSet rset11 = st11.executeQuery("select sum(db.debit),sum(db.credit),SUM(db.balance) from ac_debtors db "
                                        + "WHERE db.date BETWEEN '"+beginDate+"' AND '"+endDate+"' and db.account_no = '"+listofStaffNos[j]+"'");// and (transaction_type = 'Raise Invoice' OR transaction_type = 'Stock Delivery' OR transaction_type = 'Stock Returns') and (description = 'Creditors Payable' OR description = 'Other Payables') GROUP BY invoice_no,date,dealer,item ORDER BY date");
     ...
     public java.lang.Object[] getListofStaffNos1() {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT payer_name FROM ac_schemes  WHERE  marketing_contact  ILIKE '"+Debtor+"' ORDER BY 1 ASC");
