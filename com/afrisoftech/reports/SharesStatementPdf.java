...
public class SharesStatementPdf implements java.lang.Runnable {
    ...
    public void SharesStatementPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("This Share Statement is  for Metropolitan Health Services Limited Ksh 5 ordinary shares - Page: ",pFontHeader), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
                    ...
                    try {
                        ...
                        int headerwidths[] = {13,20,8,9,14,14,15};
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset4 = st4.executeQuery("select holders_name,acc_no ,tel1,postal_code,postal_address from pb_share_register where acc_no = '"+memNo+"'");
                            java.sql.ResultSet rset1 = st1.executeQuery("select date,item,reference_no,gl_code,debit,credit from ac_shares_ledger where account_no = '"+memNo+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and transaction_type ilike 'sha%' order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                            java.sql.ResultSet rsetT = st41.executeQuery("select sum(credit-debit) from ac_shares_ledger where account_no = '"+memNo+"' AND date::date < '"+beginDate+"' and transaction_type ilike 'sha%'");
                            ...
                            phrase = new Phrase("Share Statement", pFontHeader11);
                            table.addCell(phrase);
                            ...
                            while (rset4.next()){
                                ...
                                table.addCell(phrase);
                                phrase = new Phrase("Tel No : " +dbObject.getDBObject(rset4.getObject(3), "-"), pFontHeader1);
                                table.addCell(phrase);
                                phrase = new Phrase("Postal Code : " +dbObject.getDBObject(rset4.getObject(4), "-"), pFontHeader1);
                                table.addCell(phrase);
                                phrase = new Phrase("Address: " +dbObject.getDBObject(rset4.getObject(5), "-"), pFontHeader1);
                                table.addCell(phrase);
                            }
                            table.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                            ...
                            table.addCell(phrase);
                            phrase = new Phrase("Account",pFontHeader1);
                            table.addCell(phrase);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                            
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Sold",pFontHeader1);
                            table.addCell(phrase);
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Bought",pFontHeader1);
                            table.addCell(phrase);
                            ...
                            while (rset1.next()){
                                ...
                                table.addCell(phrase);
                                osBalancebf = (osBalance+osBalanceOs-osBalance1);
                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(osBalance+osBalanceOs-osBalance1)),pFontHeader);
