...
public class DetailedPatientFinalInvpdf implements java.lang.Runnable {
    ...
    public void DetailedPatientFinalInvpdf(java.sql.Connection connDb, java.lang.String combox,java.lang.String invoiceNO,java.lang.String payer) {
        MNo = combox;
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            String visitId = memNo,payer=null,payeraccount=null,memberNO="-",patName=null;
                            
                            System.out.println("select dealer,account_no from ac_debtors where dealer ilike '"+payer1+"' and invoice_no='"+invoiceno+"'");

                             java.sql.ResultSet rset311 = st22.executeQuery(""
                                     + "select dealer,account_no from ac_debtors where dealer ilike '"+payer1+"' and invoice_no='"+invoiceno+"'");
                                 while (rset311.next()){
                                     payer = rset311.getObject(1).toString();
                                     payeraccount = rset311.getObject(2).toString();
                                 }
                                   st22 = connectDB.createStatement();
                                 switch(payer1){
                                     case "NHIF":
                                     { java.sql.ResultSet rset3112 = st22.executeQuery(""
                                     + "select card_no from hp_patient_card where card_name ilike 'N.H.I.F'  and visit_id='" + visitId + "' and invoice_no='"+invoiceno+"'");
                                 while (rset3112.next()){
                                     memberNO = rset3112.getObject(1).toString();
                                 }                             }
                                 break;
                                     default:{
                                         memberNO="-";
                            ...
                            phrase = new Phrase("Detailed Final Invoice".toUpperCase(), pFontHeaderX);
                            table1.addCell(phrase);
                            table1.getDefaultCell().setFixedHeight(16);
                            table1.getDefaultCell().setBorder(Rectangle.BOX);
                            table1.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                            ...
                            while (rset.next()) {
                                ...
                                phrase = new Phrase("Payer Acc: ".toUpperCase() + dbObject.getDBObject(payeraccount, "-").toUpperCase(), pFontHeader1);
                                table1.addCell(phrase);
                                ...
                                table1.getDefaultCell().setColspan(3);
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase1 = new Phrase("Scheme Name : ".toUpperCase() + dbObject.getDBObject(payer, "-").toUpperCase(), pFontHeader1);
                                //table1.addCell(phrase);
                               ...
                               patName= dbObject.getDBObject(rset.getObject(2), "-").toUpperCase();
                                //table1.addCell(phrase);
                            }

                            while (rseta.next()) {
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Invoice No.: ".toUpperCase() + invoiceno.toUpperCase(), pFontHeader1);
                                table1.addCell(phrase);
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase4 = new Phrase("Member Name: ".toUpperCase() + patName, pFontHeader1);
                                //table1.addCell(phrase);
                         ...
                         //   while (rsetb.next())
                            {
                                table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Member No.: ".toUpperCase() + dbObject.getDBObject(memberNO, "-").toUpperCase(), pFontHeader1);
                                table1.addCell(phrase);
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery(" select initcap(main_service) as service,sum(dosage)::int,sum(debit-credit) from hp_patient_card where visit_id = '" + memNo + "' and service != 'N.H.I.F' AND service != 'N.H.I.F' AND service not ilike 'Receipt%' AND transaction_type not ilike 'Receipts' AND transaction_type not ilike 'Receipt' AND main_service not ilike 'Receipts' AND service not ilike 'Receipt Adj'  AND service not ilike '%DIFF ' AND service not ilike 'Discou%' AND service != 'Invoice' group by initcap(main_service) order by service"); //having sum(debit-credit)>0 order by service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            //java.sql.ResultSet rset1 = st1.executeQuery(" select initcap(main_service) as service,sum(dosage)::int,sum(debit-credit) from hp_patient_card where visit_id = '" + memNo + "' and service != 'N.H.I.F' AND service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj'  AND service not ilike '%DIFF ' AND service not ilike 'Discou%' AND service != 'Invoice' group by initcap(main_service)  order by service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
...
//                               java.sql.ResultSet rset1a = st1a.executeQuery("  select date::date ,initcap(service) as service,dosage::int,sum(debit-credit) from hp_patient_card"
//                                       + " where visit_id = '"+MNo+"' and service != 'N.H.I.F' AND service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj'  AND service not ilike '%DIFF ' AND service not ilike 'Discou%' AND service != 'Invoice' and main_service ilike '"+rset1.getString(1)+"' "
//                                       + "group by 2,3,1 having sum(debit-credit) >0 order by date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
//                            
                                  
                                  java.sql.ResultSet rset1a = st1a.executeQuery("  select date::date ,initcap(service) as service,dosage::int,sum(debit-credit) from hp_patient_card"
                                       + " where visit_id = '"+MNo+"' and service != 'N.H.I.F' AND service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj'  AND service not ilike '%DIFF ' AND service not ilike 'Discou%' AND service != 'Invoice' and main_service ilike '"+rset1.getString(1)+"' "
                                       + "group by 2,3,1 order by date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(dbObject.getDBObject(rset1a.getObject(1), "-"), pFontHeader);

                                table.addCell(phrase);

                                table.getDefaultCell().setColspan(2);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(dbObject.getDBObject(rset1a.getObject(2), "-"), pFontHeader);
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                int dos1 = 0;
                                dos1 = rset1a.getInt(3);

                                phrase = new Phrase(java.lang.String.valueOf(dos1), pFontHeader);
                               ...
                               // phrase = new Phrase(dbObject.getDBObject(rset1.getObject(2), "-"), pFontHeader);
                                ...
                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rset1a.getString(4)), pFontHeader);

                                table.addCell(phrase);
                                table.getDefaultCell().setColspan(1);
                                osBalance = osBalance + rset1a.getDouble(4);
                                subtotal=subtotal+ rset1a.getDouble(4);
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Sub Total - "+rset1.getString(1), pFontHeader1);               
                                table.addCell(phrase);
                            ...
                            while (rsetTotals.next()) {
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Total Billed Amount".toUpperCase(), pFontHeader1);
                            ...
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            table.getDefaultCell().setColspan(6);
                            phrase = new Phrase("BILL SETTLEMENT DETAILS", pFontHeader1);
                            ...
                            while (rsetTotals1.next()) {
                               ...
                               // table.getDefaultCell().setColspan(2);
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Total Settled Amount".toUpperCase(), pFontHeader1);
                            ...
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Balance Due / Payable ".toUpperCase(), pFontHeader1);
                            ...
                            //  phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rsetTotals.getString(1)),pFontHeader);
                            // table.addCell(phrase);
                            phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf((osBalance - osBalance1) - deposit)), pFontHeader1);
                            ...
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Total Un-Utilised Deposits".toUpperCase(), pFontHeader1);
