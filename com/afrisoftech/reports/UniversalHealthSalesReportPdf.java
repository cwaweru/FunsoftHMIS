                    ...
                    try {
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                java.sql.PreparedStatement psetw = connectDB.prepareStatement("SELECT sum(debit-credit) FROM hp_patient_card WHERE invoice_no IN (SELECT invoice_no FROM ac_debtors WHERE date = '" + listofAct[i] + "' AND payee = upper('"+schemeName+"')) AND UPPER(transaction_type) = UPPER('Billing') AND UPPER(main_service) IN (SELECT DISTINCT UPPER(activity) FROM pb_activity WHERE uhc_tag = 'AOT')");
...
//                                table.addCell(phrase);
    ...
    public java.lang.Object[] getListofActivities1() {
        ...
        try {
            ...
            java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT code FROM pb_activity WHERE uhc_tag = 'AOT' GROUP BY code ORDER BY code");
