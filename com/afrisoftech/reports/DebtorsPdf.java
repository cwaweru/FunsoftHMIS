...
public class DebtorsPdf implements java.lang.Runnable {
    ...
    public void DebtorsPdf(java.sql.Connection connDb,java.util.Date begindate,java.util.Date endate, java.lang.String bal) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                java.sql.PreparedStatement pSeta1 = connectDB.prepareStatement("SELECT count(account_no) FROM ac_debtors"
                                        + " where  account_no = '"+listofAct[i]+"' order by 1 ASC");
                                java.sql.PreparedStatement pset22 = connectDB.prepareStatement("select distinct account_no||' '||scheme_name from ac_schemes "
                                        + "WHERE account_no = ? order by 1 ASC");
                                ...
                                if (name > 0){
                                    ...
                                    while (rset22.next()){
                                        ...
                                        java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(balance) from ac_debtors "
                                                + "WHERE account_no = ?  AND date <= '"+ageingDates[0][0]+"'");
                                        pset1.setString(1,listofAct[i].toString());
                                        java.sql.ResultSet rset1 = pset1.executeQuery();
                                        
                                        java.sql.PreparedStatement pset111 = connectDB.prepareStatement("select sum(balance-credit_bal) from ac_debtors "
                                                + "WHERE account_no = ?  AND date <= '"+endDate+"'");
                                        ...
                                        java.sql.PreparedStatement pset112 = connectDB.prepareStatement("select sum(credit_bal) from ac_debtors"
                                                + " WHERE account_no = ? AND date <= '"+endDate+"'");
                                        ...
                                        for (int t = ageingDates.length - 1; t >= 0; t--) {
                                            ...
                                            java.sql.PreparedStatement pset = connectDB.prepareStatement("select sum(balance) from ac_debtors"
                                                    + " WHERE account_no = ?  AND date between '"+ageingDates[t][0]+"' AND '"+ageingDates[t][1]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
            ...
            if(balnc.equalsIgnoreCase("both")){
                java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT CASE WHEN (account_no IS NULL) THEN '-' ELSE account_no END AS account_no,sum(balance-credit_bal) as balance FROM ac_debtors WHERE date::date <= '"+endDate+"' group by account_no HAVING sum(balance-credit_bal) <> 0 order by 1 asc");
