...
public class InPatientIntmDateBflnvPdf implements java.lang.Runnable {
    ...
    public void InPatientIntmDateBflnvPdf(java.sql.Connection connDb, java.lang.String combox,java.util.Date begindate,java.lang.String pNo) {
        MNo = combox;
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.Statement st2 = connectDB.createStatement();
                            java.sql.Statement st1bf = connectDB.createStatement();
                            ...
                            java.sql.ResultSet rsetTotals1 = st21.executeQuery("select sum(credit) from hp_patient_card where visit_id = '"+memNo+"' and (service = 'N.H.I.F' or service ilike 'Receipt' or service ilike 'Receipt Adj'  OR service ilike 'Discou%') AND patient_no = '"+patNo+"'");
                            java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(debit - credit) from hp_patient_card where visit_id = '"+memNo+"' AND service != 'N.H.I.F' AND service not ilike 'Receipt' AND service not ilike 'Receipt Adj'  AND service not ilike 'Discou%' AND service != 'Invoice' and paid = 'true' AND patient_no = '"+patNo+"'");
                            
                            java.sql.ResultSet rset11 = st11.executeQuery(" select date::date,initcap(service) as service,dosage::int,CASE WHEN (service = 'Receipt') THEN requisition_no ELSE reference END AS reference,credit from hp_patient_card where visit_id = '"+memNo+"' AND (service = 'N.H.I.F' or service ilike 'Receipt' or service ilike 'Receipt Adj'  OR service ilike 'Discou%') AND patient_no = '"+patNo+"' order by date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            
                            java.lang.Object listofStaffNos1[] = this.getListofStaffNos1();
                            java.sql.ResultSet rset1bf = st1bf.executeQuery(" select sum(debit-credit) from hp_patient_card where visit_id = '"+memNo+"' and service != 'N.H.I.F' AND service not ilike 'Receipt' AND service not ilike 'Receipt Adj' AND service not ilike 'Discou%' AND service != 'Invoice'  and date::date <'"+beginDate+"' AND patient_no = '"+patNo+"'");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            ...
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                            table.getDefaultCell().setColspan(3);
                            phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rset1bf.getString(1)),pFontHeader1);
                            osBalance = osBalance + rset1bf.getDouble(1);
                            table.addCell(phrase);
                            for (int k = 0; k < listofStaffNos1.length; k++) {
                                double osBalance1 = 0.00;
                                java.sql.ResultSet rset1 = st1.executeQuery(" select date::date,initcap(service) as service,dosage::int,reference,debit-credit from hp_patient_card where visit_id = '"+memNo+"' and service != 'N.H.I.F' AND service not ilike 'Receipt' AND service not ilike 'Receipt Adj' AND service not ilike 'Discou%' AND service != 'Invoice'  AND date::date = '"+listofStaffNos1[k]+"' order by date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
...
docPdf.close();  com.afrisoftech.lib.PDFRenderer.renderPDF(tempFile);
    ...
    public java.lang.Object[] getListofStaffNos1() {
        ...
        try {
            ...
            java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT distinct date::date as date FROM hp_patient_card WHERE visit_id = '"+MNo+"' and debit > 0  AND patient_no = '"+patNo+"' order by date");
