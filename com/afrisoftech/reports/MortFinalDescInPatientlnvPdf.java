...
public class MortFinalDescInPatientlnvPdf implements java.lang.Runnable {
    ...
    public void MortFinalDescInPatientlnvPdf(java.sql.Connection connDb, java.lang.String invoiceNo, java.lang.String pNo) {
        MNo = invoiceNo;
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                            table1.getDefaultCell().setBorder(Rectangle.BOX);
                            phrase = new Phrase("Farewell Home Final Invoice".toUpperCase(), pFontHeader11);
                            table1.addCell(phrase);
                            table1.getDefaultCell().setBorder(Rectangle.BOX);
                            table1.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                            ...
                            while (rset.next()) {
                                ...
                                while (rseta.next()) {
                                    table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase("Invoice No. : " + dbObject.getDBObject(invoiceNumber, "-"), pFontHeader1);
                                    table1.addCell(phrase);
                                    //  while (rset.next())
                                    table1.getDefaultCell().setColspan(3);
                                    table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    ...
                                    // while (rseta.next())
                                    table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase("Invoiced By : " + dbObject.getDBObject(com.afrisoftech.lib.UserName.getUserName(connectDB), "-"), pFontHeader1);
                                    table1.addCell(phrase);
                                    // while (rset.next())
                                    table1.getDefaultCell().setColspan(3);
                                    table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
...
//                            table.addCell(phrase);
                            ...
                            for (int k = 0; k < listofStaffNos1.length; k++) {
                                double osBalance1 = 0.00;
                                java.sql.ResultSet rset1 = st1.executeQuery(" select date::date, billing_time::time, user_name, initcap(service) as service,dosage::int,reference,debit-credit from hp_patient_card where invoice_no = '" + invoiceNumber + "' "
                                        + " AND (invoice_no NOT LIKE 'O%' and invoice_no NOT LIKE 'IP%') AND (transaction_type not ilike 'Receipts%' and transaction_type not ilike 'Remittance%') and "
