...
public class PatientCardPdf implements java.lang.Runnable {
    ...
    public void PatientCardPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate, java.lang.String combox, boolean useDateRangee) {
        ...
        threadSample.start();
        pFontHeader12.setColor(BaseColor.BLUE);
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            table.addCell(phrase);
                            java.sql.ResultSet rsetp = stp.executeQuery("select distinct patient_no,initcap(first_name||' '||second_name),Category,residence,tel_no,last_visit,sex,nok, funsoft_patient_age(year_of_birth::date) as age,pay_mode,'OP' AS type from hp_patient_register where patient_no = '" + memNo + "' UNION "
                                    + "SELECT DISTINCT patient_no, initcap(first_name||' '||second_name), ''as Category,residence, tel_no,  adm_date::date as last_visit, sex, nok, (CASE WHEN year_of_birth = '' THEN null ELSE funsoft_patient_age(year_of_birth::date) END) as age, pay_mode,'IP' AS type FROM hp_inpatient_register where patient_no = '" + memNo + "'");
                            ...
                            if (type.equalsIgnoreCase("IP")) {
                                ...
                                java.sql.ResultSet rset = stmt.executeQuery("select op_no from hp_inpatient_register where patient_no = '" + MNo + "' AND op_no IS NOT NULL AND op_no != ''  ");
                                while (rset.next()) {
                                    opNo = rset.getString(1);
                                    
                                }
                                ipNo = memNo;
                            } else if (type.equalsIgnoreCase("OP")) {
                                java.sql.Statement stmt = connectDB.createStatement();
                                java.sql.ResultSet rset = stmt.executeQuery("select patient_no from hp_inpatient_register where op_no = '" + MNo + "' AND op_no IS NOT NULL AND op_no != '' ");
                                while (rset.next()) {
                                    ipNo = rset.getString(1);
                            ...
                            String typeData = "";
                            for (int typecounter = 0; typecounter < 2; typecounter++) {
                                if(typecounter == 0 ){
                                    MNo = opNo;
                                    memNo = opNo;
                                    typeData = "OUT-PATIENT DATA";
                                }
                                if(typecounter == 1 ){
                                    MNo = ipNo;
                                    memNo = ipNo;
                                    typeData = "IN-PATIENT DATA";
                                }
                                ...
                                if (!MNo.isEmpty()) {
                                    ...
                                    table.getDefaultCell().setColspan(6);
                                    table.getDefaultCell().setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
                                    phrase = new Phrase(typeData, pFontHeader12);
                                    table.addCell(phrase);
                                        ...
                                        for (int l = 0; l < listofStaffNos1.length; l++) {
                                            ...
                                            while (rset121.next()) {
                                                ...
                                                if (Tech.equalsIgnoreCase("")) {
                                                    ...
                                                    System.err.println("SELECT DISTINCT status,notes FROM pb_lab_standards where  code ilike '" + Test + "'");
                                                    while (rset4c.next()) {
                                                    ...
                                                    // while (rset1w.next()) {
                                                    System.err.println("Status for Above ---- "+Status);
                                                    if (Status.startsWith("t")) {
                                                        System.err.println("Status for Above is true");
                                                        ...
                                                        System.err.println("Status for Above is false");
                                                        table.getDefaultCell().setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
                                                        table.getDefaultCell().setColspan(1);
                                                        phrase = new Phrase(TimesD, pFontHeader);
                                                        table.addCell(phrase);
