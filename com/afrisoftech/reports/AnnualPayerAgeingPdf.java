                ...
                try {
                    ...
                    try {
                       ...
                       //  for (int i = 0; i < rangeDates.length; i++) {   
                        ...
                        phrase = new Phrase("Over "+((interval + 0) * 1)+" Years",pFontHeader);
                        table.addCell(phrase);
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                java.sql.PreparedStatement pSeta1 = connectDB.prepareStatement("SELECT count(account_no) FROM ac_debtors "
                                        + "where  account_no = '"+listofAct[i]+"' and balance > 0");
                                java.sql.PreparedStatement pset22 = connectDB.prepareStatement("select account_no||' '||scheme_name from ac_schemes"
                                        + " WHERE account_no = ? order by 1 ASC");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                if (name > 0){
                                    ...
                                    java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(balance) from ac_debtors WHERE"
                                            + " account_no = ?  AND date <= '"+rangeDates[0][0]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                    ...
                                    java.sql.PreparedStatement pset111 = connectDB.prepareStatement("select sum(balance-credit_bal) from ac_debtors"
                                            + " WHERE account_no = ?  AND date < '"+endDate+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                  ...
                                  //  for (int t = 0; t < rangeDates.length; t++) {
                                        ...
                                        java.sql.PreparedStatement pset = connectDB.prepareStatement("select sum(balance) from ac_debtors"
                                                + " WHERE account_no = ?  AND date between '"+rangeDates[t][0]+"' AND '"+rangeDates[t][1]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                    ...
                                    java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(balance) from ac_debtors "
                                            + "WHERE account_no = ?  AND date <= '"+rangeDates[0][0]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                    pset1.setString(1,listofAct[i].toString());
                                    java.sql.ResultSet rset1 = pset1.executeQuery();
