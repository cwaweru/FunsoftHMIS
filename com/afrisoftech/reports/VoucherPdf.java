...
public class VoucherPdf implements java.lang.Runnable {
    ...
    public void VoucherPdf(java.sql.Connection connDb, java.lang.String voucno, boolean printPreview) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select distinct cb.dealer from ac_cash_book cb where cb.voucher_no = '"+vouchNo+"' GROUP BY cb.dealer");
                            java.sql.ResultSet rs = st.executeQuery("select cb.voucher_no,chq_date,cb.cheque_no,initcap(cb.payment_mode) from ac_cash_book cb where cb.voucher_no = '"+vouchNo+"'");
                            java.sql.ResultSet rset5 = st5.executeQuery("select sum(cb.credit) from ac_cash_book cb where cb.voucher_no = '"+vouchNo+"'");
                            //  java.sql.ResultSet rset6 = st6.executeQuery("select cb.inv_no,cb.amount_paid,vat_amount,CASE WHEN (cb.reason IS NULL) THEN '-' ELSE cb.reason END AS reason from cr_allocation cb where cb.voucher_no = '"+vouchNo+"' UNION ALL select cb.transaction_no,cb.credit,0.00,CASE WHEN (cb.description IS NULL) THEN '-' ELSE cb.description END AS reason from ac_cash_book cb where cb.voucher_no = '"+vouchNo+"'");
                            ...
                            java.sql.ResultSet rset6x = st6x.executeQuery("select count(cb.inv_no) from cr_allocation cb where cb.voucher_no = '"+vouchNo+"'");
                            while (rset6x.next()){
                                vno = rset6x.getInt(1);
                            }
                            ...
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                            phrase = new Phrase(rset3.getObject(1).toString(), pFontHeader31);
                            table.addCell(phrase);
                            ...
                            phrase = new Phrase("Voucher No : "+  dbObject.getDBObject(rs.getObject(1), "-"), pFontHeader);
                            table.addCell(phrase);
                            
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                            phrase = new Phrase("Date of Cheque : "+ rs.getObject(2).toString(), pFontHeader1);
                            ...
                            table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Payee : " + dbObject.getDBObject(rset1.getObject(1), "-"), pFontHeader);
                            ...
                            while (rset611.next()){
                                phrase = new Phrase("Vat Cert. No. : " +  dbObject.getDBObject(rset611.getObject(1), "-"), pFontHeader1);
                            table.addCell(phrase);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            table.getDefaultCell().setColspan(6);
                            ...
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                            table.getDefaultCell().setColspan(2);
                            phrase = new Phrase("Inv. Amount "+ks, pFontHeader1);
                            table.addCell(phrase);
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Vat "+ks, pFontHeader1);
                            table.addCell(phrase);
                            table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                            
                            if(vno > 0){
                                java.sql.ResultSet rset6 = st6.executeQuery("select cb.inv_no,cb.amount_paid,vat_amount,CASE WHEN (cb.reason IS NULL) THEN '-' ELSE cb.reason END AS reason from cr_allocation cb where cb.voucher_no = '"+vouchNo+"' ORDER BY cb.reason");
                                while (rset6.next()){
                                    ...
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase(rset6.getObject(1).toString()+"  "+rset6.getObject(4).toString(), pFontHeader);
                                ...
                                java.sql.ResultSet rset6 = st6.executeQuery("select cb.transaction_no,cb.credit,0.00,CASE WHEN (cb.description IS NULL) THEN '-' ELSE cb.description END AS reason from ac_cash_book cb where cb.voucher_no = '"+vouchNo+"'");
                                while (rset6.next()){
