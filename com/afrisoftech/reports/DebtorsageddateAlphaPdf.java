...
public class DebtorsageddateAlphaPdf implements java.lang.Runnable {
    double Credit = 0.00;
    ...
    public void DebtorsageddateAlphaPdf(java.sql.Connection connDb,java.util.Date begindate,java.util.Date endate, java.lang.String bal) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                java.sql.PreparedStatement pSeta1 = connectDB.prepareStatement("SELECT count(account_no) FROM ac_debtors"
                                        + " where  account_no = '"+listofAct[i]+"'");
                                java.sql.PreparedStatement pset22 = connectDB.prepareStatement("select distinct account_no||' '||scheme_name from "
                                        + "ac_schemes WHERE account_no = ? order by 1 ASC");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                if (name > 0){
                                    ...
                                    while (rset22.next()){
                                        ...
                                        java.sql.PreparedStatement pset111 = connectDB.prepareStatement("select sum(debit-credit) from ac_debtors "
                                        ...
                                        java.sql.PreparedStatement pset112 = connectDB.prepareStatement("select sum(credit) from ac_debtors"
                                                + " WHERE account_no = ? AND date <= '"+endDate+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                        ...
                                        for (int t = ageingDates.length - 1; t >= 0; t--) {
                                            ...
                                            java.sql.PreparedStatement pset = connectDB.prepareStatement("select sum(debit),invoice_no from ac_debtors "
                                                    + "WHERE account_no = ?  AND date between '"+ageingDates[t][0]+"' AND '"+ageingDates[t][1]+"' "
                                            ...
                                            while (rset.next()){
                                                ...
                                                java.sql.PreparedStatement psetx = connectDB.prepareStatement("select sum(credit),invoice_no from ac_debtors"
                                                        + " WHERE account_no = ?  AND date <= '"+endDate+"' AND invoice_no = '"+invoiceNo+"'  GROUP BY invoice_no order by 2 ASC ");
