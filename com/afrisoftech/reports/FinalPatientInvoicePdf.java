                    ...
                    try {
                        ...
                        for (int j = 0; j < listofStaffNos.length; j++) {
                            ...
                            try {
                                ...
                                java.sql.ResultSet rset32 = st32.executeQuery("select DISTINCT pc.payee,sp.postal_code||' '||sp.address,sp.tel_main||' '||sp.other_tel,sp.main_fax from ac_schemes sp,ac_debtors pc where pc.payee = sp.scheme_name and pc.invoice_no = '"+listofStaffNos[j]+"' AND pc.date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                                java.sql.ResultSet rset31 = st31.executeQuery("select DISTINCT pc.payee,sp.postal_code||' '||sp.address,sp.tel_main||' '||sp.other_tel,sp.main_fax from ac_schemes sp,ac_debtors pc where pc.payee = sp.scheme_name and pc.invoice_no = '"+listofStaffNos[j]+"' AND pc.date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                                java.sql.ResultSet rset4 = st4.executeQuery("select DISTINCT sp.scheme_manager,sp.postal_code||' '||sp.address,sp.tel_main||' '||sp.other_tel,sp.main_fax from ac_scheme_providers sp,ac_debtors pc where pc.dealer = sp.scheme_manager and pc.invoice_no = '"+listofStaffNos[j]+"'");
                                java.sql.ResultSet rset5 = st5.executeQuery("select DISTINCT pr.account_no ,pr.member_name  from hp_patient_register pr,ac_debtors ac where pr.patient_no = ac.admission_no and ac.invoice_no = '"+listofStaffNos[j]+"'");
                                java.sql.ResultSet rset = st.executeQuery("select DISTINCT pr.patient_no,initcap(pr.first_name||' '||pr.second_name||' '||pr.last_name),pr.address,pr.residence,pr.tel_no from hp_patient_register pr,  ac_debtors ac where pr.patient_no = ac.admission_no and ac.invoice_no = '"+listofStaffNos[j]+"'");
                                java.sql.ResultSet rset1 = st1.executeQuery("select date::date,initcap(service) as service,reference,dosage,debit from hp_patient_card where invoice_no = '"+listofStaffNos[j]+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND debit != 0 order by invoice_no");
                                ...
                                java.sql.ResultSet rset12 = st12.executeQuery("select DISTINCT date from ac_debtors where invoice_no = '"+listofStaffNos[j]+"' and date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                                java.sql.ResultSet rset11 = st11.executeQuery("select DISTINCT invoice_no from ac_debtors where invoice_no = '"+listofStaffNos[j]+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and invoice_no IS NOT NULL");
...
docPdf.close();  com.afrisoftech.lib.PDFRenderer.renderPDF(tempFile);
    ...
    public java.lang.Object[] getListofStaffNos() {
        ...
        try {
            ...
            java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT invoice_no FROM ac_debtors WHERE date  BETWEEN ? AND ? AND transaction_type = ? AND payee != ? order by invoice_no");
