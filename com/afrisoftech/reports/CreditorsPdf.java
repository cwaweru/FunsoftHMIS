...
public class CreditorsPdf implements java.lang.Runnable {
    ...
    public void CreditorsPdf(java.sql.Connection connDb,java.util.Date begindate,java.util.Date endate, java.lang.String bType, java.lang.String actCode) {
        balType = bType;
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                if (name > 0){
                                    ...
                                    while (rset22.next()){
                                        ...
                                        table.getDefaultCell().setColspan(1);
                                        //  table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                        phrase = new Phrase(dbObject.getDBObject(rset22.getObject(1), "-").toUpperCase(),pFontHeader1);
                                        table.addCell(phrase);
                                    ...
                                    for (int t = ageingDates.length - 1; t >= 0; t--) {
                                        ...
                                        java.sql.PreparedStatement pset = connectDB.prepareStatement("select sum(credit),invoice_no from ac_accounts_payable WHERE dealer = ? AND date between '"+ageingDates[t][0]+"' AND '"+ageingDates[t][1]+"' and credit > 0  AND activity_code = '"+activityCode+"' group by invoice_no");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
...
docPdf.close();  com.afrisoftech.lib.PDFRenderer.renderPDF(tempFile);
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
            ...
            // java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT dealer FROM ac_accounts_payable where dealer IS NOT NULL order by dealer");
            
            if(balType.equalsIgnoreCase("both")){
                java.sql.Statement stmt1 = connectDB.createStatement();
                java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT CASE WHEN (dealer IS NULL) THEN '-' ELSE dealer END AS dealer, sum(credit-debit) FROM ac_accounts_payable WHERE date::date <= '"+endDate+"' AND activity_code = '"+activityCode+"' GROUP BY  dealer HAVING sum(credit-debit) <> 0 order by dealer");
           /////      java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT CASE WHEN (dealer IS NULL) THEN '-' ELSE dealer END AS dealer, sum(credit-debit) FROM ac_accounts_payable WHERE date::date <= '"+endDate+"'  GROUP BY  dealer HAVING sum(credit-debit) <> 0 order by 2 desc");
                ...
                if(balType.equalsIgnoreCase("neg")){
                    java.sql.Statement stmt1 = connectDB.createStatement();
                    java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT dealer FROM creditors_account where dealer IS NOT NULL AND bal < 0 AND activity_code = '"+activityCode+"' order by dealer");
